...
public class NussinovDesignDemo extends JFrame
    implements InterfaceVARNAListener, InterfaceVARNABasesListener, ItemListener {
  ...
  private static final String SEQUENCE_BIG =
      "AAAACAAAAACACCAUGGUGUUUUCACCCAAUUGGGUGAAAACAGAGAUCUCGAGAUCUCUGUUUUUGUUUU";
  ...
  private void RNAPanelDemoInit() {
    ...
    _switchButton.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
    ...
    _vpMaster.setTitle("Meilleur repliement - S�quence courante");
    ...
    setTitle("F�te de la science 2015 - Inria AMIB - Design d'ARN");
  ...
  private synchronized void showSolution() {
    ...
    if ((sols.size() == 1) && (sols.get(0).equals(_struct.getSelectedItem().toString()))) {
      ...
      "Vous avez trouv� une s�quence pour cette structure !!!\n Saurez vous faire le design de mol�cules plus complexes ?",
      "F�licitations !",
      JOptionPane.INFORMATION_MESSAGE);*/

    } else {
      this._vpMaster.setTitle("Meilleur repliement - S�quence courante");
    }
  }
        ...
        if (i < j) {
          ...
          for (int k = i + 1; k <= j; k++) {
            if (canBasePair(seq.charAt(i), seq.charAt(k))) {
              ...
              tab[i][j] =
                  Math.max(tab[i][j], basePairScore(seq.charAt(i), seq.charAt(k)) + fact1 + fact2);
            }
          }
        }
      }
  ...
  public static ArrayList<Double> combine(
      double bonus, ArrayList<Double> part1, ArrayList<Double> part2) {
    ...
    public void setSelectedIndex(int i) {
      ...
      try {
        ...
        if ((_sols.size() == 1) && (target.getStructDBN().equals(_sols.get(0))))
          rfolded.setName("F�licitations !");
        else rfolded.setName("Repliement stable - " + (i + 1) + "/" + _sols.size());
        _vpMaster.showRNAInterpolated(rfolded);
        _vpTarget.repaint();
      } catch (ExceptionUnmatchedClosingParentheses e) {
        // TODO Auto-generated catch block
