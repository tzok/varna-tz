...
public class VARNAGUI extends JFrame
    implements DropTargetListener, InterfaceVARNAListener, MouseListener, AdjustmentListener {

  /** */
  private static final long serialVersionUID = -790155708306987257L;

  private static final String DEFAULT_SEQUENCE =
  ...
  private void RNAPanelDemoInit() {
    ...
    _sideList.addListSelectionListener(
    ...
    JScrollPane listScroller =
        new JScrollPane(
            _sideList); // ,JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
          ...
          public void actionPerformed(ActionEvent e) {
            try {
              ...
              JOptionPane.showMessageDialog(
                  _vp, e1.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    ...
    _duplicateButton.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            _rnaList.add(
                (VARNAConfig) _vp.getConfig().clone(),
                _vp.getRNA().clone(),
                _vp.getRNA().getName()
                    + "-"
    ...
    _vp.addSelectionListener(
        new InterfaceVARNASelectionListener() {
          ...
          public void onSelectionChanged(
              BaseList selection, BaseList addedBases, BaseList removedBases) {
  ...
  public static void main(String[] args) {
    ...
    // ""+Toolkit.getDefaultToolkit().getImage("./VARNA16x16.png"), "Check",
  ...
  public void onTranslationChanged() {
    if (_vp.getZoom() > 1.02) {
      int nx =
          _horiz.getMaximum() - (_vp.getTranslation().x - _horiz.getMinimum()) - _vp.getWidth();
  ...
  public void adjustmentValueChanged(AdjustmentEvent arg0) {
    if (arg0.getSource() == _horiz) {
      _vp.setTranslation(
          new Point(
              _horiz.getMaximum() - (arg0.getValue() - _horiz.getMinimum()) - _vp.getWidth(),
              _vp.getTranslation().y));
      _vp.repaint();
    } else if (arg0.getSource() == _vert) {
      _vp.setTranslation(
          new Point(
              _vp.getTranslation().x,
              _vert.getMaximum() - (arg0.getValue() - _vert.getMinimum()) - _vp.getHeight()));
      _vp.repaint();
    }
  }
}
