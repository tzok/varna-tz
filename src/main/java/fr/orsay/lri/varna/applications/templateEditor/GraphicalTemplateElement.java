...
public abstract class GraphicalTemplateElement {
  ...
  public void attach(
      GraphicalTemplateElement e, RelativePosition edgeOrig, RelativePosition edgeDest)
      throws ExceptionEdgeEndpointAlreadyConnected, ExceptionInvalidRNATemplate {
    _attachedElements.put(
        edgeOrig, new Couple<RelativePosition, GraphicalTemplateElement>(edgeDest, e));
  }

  /**
   * Same as attach(), but without touching the underlying RNATemplateElement objects. This is
   * useful if the underlying RNATemplate already contains edges but not the graphical template.
   */
  public void graphicalAttach(
      GraphicalTemplateElement e, RelativePosition edgeOrig, RelativePosition edgeDest) {
    _attachedElements.put(
        edgeOrig, new Couple<RelativePosition, GraphicalTemplateElement>(edgeDest, e));
  }
  ...
  public Couple<RelativePosition, GraphicalTemplateElement> getAttachedElement(
      RelativePosition localedge) {
    if (_attachedElements.containsKey(localedge)) return _attachedElements.get(localedge);
    return null;
  }
  ...
  private HashMap<RelativePosition, Couple<RelativePosition, GraphicalTemplateElement>>
      _attachedElements =
          new HashMap<RelativePosition, Couple<RelativePosition, GraphicalTemplateElement>>();
  ...
  public void drawStringCentered(Graphics2D g2, String res, double x, double y) {
    ...
    if (_debug) g2.drawRect((int) x, (int) y - d.height, d.width, d.height);
    g2.drawString(res, (int) Math.round(x), (int) Math.round(y));
  }

  protected Stroke _boldStroke =
      new BasicStroke(2.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 3.0f);
  protected Stroke _solidStroke =
      new BasicStroke(1.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 3.0f);
  private float[] dash = {5.0f, 5.0f};
  protected Stroke _dashedStroke =
  ...
  public abstract RelativePosition relativePositionFromEdgeEndPointPosition(
      EdgeEndPointPosition endPointPosition);

  public static boolean canConnect(
      GraphicalTemplateElement el1,
      RelativePosition e1,
      GraphicalTemplateElement el2,
      RelativePosition e2) {
    return (!el1.hasAttachedElement(e1))
        && (!el2.hasAttachedElement(e2))
        && (el1.isIn(e1) != el2.isIn(e2));
  }
  ...
  protected void drawMove(Graphics2D g2d, Point2D.Double center) {
    ...
    g2d.fillOval(
        (int) ((center.x - Helix.MOVE_RADIUS)),
        (int) ((center.y - Helix.MOVE_RADIUS)),
        (int) (2.0 * Helix.MOVE_RADIUS),
        (int) (2.0 * Helix.MOVE_RADIUS));
  ...
  protected void drawEditStart(
      Graphics2D g2d, Helix h, double dx, double dy, double nx, double ny) {
  ...
  protected void drawEdit(
      Graphics2D g2d, Point2D.Double center, double dx, double dy, double nx, double ny) {
    g2d.setColor(CONTROL_COLOR);
    g2d.fillOval(
        (int) ((center.x - Helix.EDIT_RADIUS)),
        (int) ((center.y - Helix.EDIT_RADIUS)),
        (int) (2.0 * Helix.EDIT_RADIUS),
        (int) (2.0 * Helix.EDIT_RADIUS));
    g2d.setColor(BACKBONE_COLOR);
    g2d.drawOval(
        (int) ((center.x - Helix.EDIT_RADIUS)),
    ...
    drawArrow(
        g2d,
        center,
        new Point2D.Double(center.x + nx * arrowLength, center.y + ny * arrowLength),
        width);
    drawArrow(
        g2d,
        center,
    ...
    drawArrow(
        g2d,
        center,
        new Point2D.Double(center.x + dx * arrowLength, center.y + dy * arrowLength),
        width);
    drawArrow(
        g2d,
        center,
  ...
  protected void drawArrow(Graphics2D g2d, Point2D.Double orig, Point2D.Double dest, double width) {
    ...
    g2d.drawLine(
        (int) dest.x,
        (int) dest.y,
        (int) (dest.x - width * (-dx + nx)),
        (int) (dest.y - width * (-dy + ny)));
    g2d.drawLine(
        (int) dest.x,
        (int) dest.y,
        (int) (dest.x - width * (-dx - nx)),
  ...
  protected void drawAnchor(Graphics2D g2d, Point2D.Double p, Color c) {
    g2d.setColor(c);
    g2d.fillOval(
        (int) (p.x - Helix.EDGE_BASE_RADIUS),
        (int) (p.y - Helix.EDGE_BASE_RADIUS),
        (int) (2.0 * Helix.EDGE_BASE_RADIUS),
        (int) (2.0 * Helix.EDGE_BASE_RADIUS));
  ...
  protected void drawBase(Graphics2D g2d, Point2D.Double p) {
    ...
    g2d.fillOval(
        (int) (p.x - Helix.BASE_RADIUS),
        (int) (p.y - Helix.BASE_RADIUS),
        (int) (2.0 * Helix.BASE_RADIUS),
        (int) (2.0 * Helix.BASE_RADIUS));
    g2d.setColor(BASE_COLOR);
    g2d.drawOval(
        (int) (p.x - Helix.BASE_RADIUS),
