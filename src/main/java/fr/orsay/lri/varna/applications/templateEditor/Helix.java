...
public class Helix extends GraphicalTemplateElement {
  ...
  public Helix(
      double x, double y, RNATemplate tmp, List<GraphicalTemplateElement> existingRNAElements) {
  ...
  private static String getNextAutomaticCaption(
      List<GraphicalTemplateElement> existingRNAElements) {
  ...
  /**
   * When an helix is moved/resized/etc... it is necessary to update the positions of endpoints from
   * unpaired regions that are attached to the helix. This function updates the endpoints positions
   * of attached unpaired regions.
   */
  ...
  public void moveCenter(double x, double y) {
    Point2D.Double center =
        new Point2D.Double(
            (_h.getStartPosition().x + _h.getEndPosition().x) / 2.0,
            (_h.getStartPosition().y + _h.getEndPosition().y) / 2.0);
    double dx = x - center.x;
    double dy = y - center.y;
    _h.setStartPosition(
        new Point2D.Double(_h.getStartPosition().x + dx, _h.getStartPosition().y + dy));
  ...
  public Point2D.Double getAbsStart5() {
    double dx =
        (_h.getStartPosition().x - _h.getEndPosition().x)
            / (_h.getStartPosition().distance(_h.getEndPosition()));
    double dy =
        (_h.getStartPosition().y - _h.getEndPosition().y)
            / (_h.getStartPosition().distance(_h.getEndPosition()));
    double nx = dy;
    double ny = -dx;
    Point2D.Double start5 =
        new Point2D.Double(
            (getPosX() - Helix.BASE_PAIR_DISTANCE * nx / 2.0),
            (getPosY() - Helix.BASE_PAIR_DISTANCE * ny / 2.0));
    return start5;
  }
  ...
  public Point2D.Double getAbsEnd5() {
    ...
    Point2D.Double end5 =
        new Point2D.Double(
            (getExtentX() - Helix.BASE_PAIR_DISTANCE * nx / 2.0),
            (getExtentY() - Helix.BASE_PAIR_DISTANCE * ny / 2.0));
    return end5;
  }
  ...
  public Point2D.Double getStart5() {
    if (_h.isFlipped()) return getAbsStart3();
    else return getAbsStart5();
  }

  public Point2D.Double getStart3() {
    if (_h.isFlipped()) return getAbsStart5();
    else return getAbsStart3();
  }

  public Point2D.Double getEnd5() {
    if (_h.isFlipped()) return getAbsEnd3();
    else return getAbsEnd5();
  }

  public Point2D.Double getEnd3() {
    if (_h.isFlipped()) return getAbsEnd5();
    else return getAbsEnd3();
  }
  ...
  public Point2D.Double getCenter() {
    return new Point2D.Double(
        (int) ((_h.getStartPosition().x + _h.getEndPosition().x) / 2.0),
  ...
  public Point2D.Double getCenterEditStart() {
    ...
    return new Point2D.Double(
        (int) (_h.getStartPosition().x + (dist - 10.0) * dx),
  ...
  public Point2D.Double getCenterEditEnd() {
    ...
    double dy = (_h.getEndPosition().y - _h.getStartPosition().y) / (dist);
    return new Point2D.Double(
        (int) (_h.getStartPosition().x + (10.0) * dx),
  ...
  public Shape getSelectionBox() {
    ...
    Point2D.Double start5 =
        new Point2D.Double(
            hbox.xpoints[0] + SELECTION_RADIUS * (dx + nx),
            hbox.ypoints[0] + SELECTION_RADIUS * (dy + ny));
    Point2D.Double end5 =
        new Point2D.Double(
            hbox.xpoints[1] + SELECTION_RADIUS * (-dx + nx),
            hbox.ypoints[1] + SELECTION_RADIUS * (-dy + ny));
    Point2D.Double end3 =
        new Point2D.Double(
            hbox.xpoints[2] + SELECTION_RADIUS * (-dx - nx),
            hbox.ypoints[2] + SELECTION_RADIUS * (-dy - ny));
    ;
    Point2D.Double start3 =
        new Point2D.Double(
            hbox.xpoints[3] + SELECTION_RADIUS * (dx - nx),
            hbox.ypoints[3] + SELECTION_RADIUS * (dy - ny));
    ;
    p.addPoint((int) start5.x, (int) start5.y);
    p.addPoint((int) end5.x, (int) end5.y);
    p.addPoint((int) end3.x, (int) end3.y);
  ...
  public RelativePosition getRelativePosition(double x, double y) {
    ...
    } else return RelativePosition.RP_OUTER;
  }
  ...
  public void detach(RelativePosition edge) {
    ...
    // Call the parent class detach function, which will also take care to disconnect this other
  ...
  public RelativePosition relativePositionFromEdgeEndPointPosition(EdgeEndPointPosition pos) {
    switch (pos) {
      case IN1:
        return RelativePosition.RP_CONNECT_START5;
      case OUT1:
