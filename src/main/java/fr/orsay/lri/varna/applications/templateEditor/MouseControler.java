...
public class MouseControler implements MouseListener, MouseMotionListener, MouseWheelListener {
  ...
  /**
   * Get mouse position in logical coordinates, ie. those used in the template XML file. It differs
   * from the screen coordinates relative to panel because of the scaling factor.
   */
  public Point2D.Double getLogicalMouseCoords(MouseEvent event) {
    return new Point2D.Double(
        event.getX() / _sp.getScaleFactor(), event.getY() / _sp.getScaleFactor());
  }
  ...
  public void mousePressed(MouseEvent arg0) {
    ...
    if (arg0.getButton() == MouseEvent.BUTTON2) {
      ...
      GraphicalTemplateElement elem =
          _sp.getElementAt(logicalMousePos.getX(), logicalMousePos.getY());
      _sp.Unselect();
      ...
      if (elem == null) {
        if (arg0.getButton() == MouseEvent.BUTTON1
          ...
          UnpairedRegion n =
              new UnpairedRegion(logicalMousePos.getX(), logicalMousePos.getY(), _sp.getTemplate());
        ...
        switch (_currentMode) {
          ...
          case RP_CONNECT_START3:
            {
              Couple<GraphicalTemplateElement, GraphicalTemplateElement.RelativePosition> al =
                  _sp.getPartner(elem, _currentMode);
              ...
              if (_elem instanceof Helix) {
                _sp.setPointerPos(
                    new Point2D.Double(logicalMousePos.getX(), logicalMousePos.getY()));
  ...
  public void mouseReleased(MouseEvent arg0) {
    ...
    if (_elem != null) {
      switch (_currentMode) {
        ...
        case RP_CONNECT_START3:
          {
            GraphicalTemplateElement t =
                _sp.getElementAt(logicalMousePos.getX(), logicalMousePos.getY(), _elem);
            if (t != null) {
              GraphicalTemplateElement.RelativePosition edge =
                  t.getClosestEdge(logicalMousePos.getX(), logicalMousePos.getY());
  ...
  public void mouseDragged(MouseEvent arg0) {
    if (movingView) {
      Point trans =
          new Point(
              arg0.getXOnScreen() - _clickedPosScreen.x, arg0.getYOnScreen() - _clickedPosScreen.y);
          ...
          case RP_EDIT_START:
            {
              if (_clickedPos.distance(arg0.getX(), arg0.getY()) > HYSTERESIS_DISTANCE) {
                ...
                Helix h1 =
                    new Helix(
                        logicalMousePos.getX(),
                        logicalMousePos.getY(),
                        _sp.getTemplate(),
                        _sp.getRNAComponents());
        ...
        if (_elem instanceof Helix) {
          ...
          switch (_currentMode) {
            case RP_EDIT_START:
              {
                Point2D.Double d =
                    projectPoint(logicalMousePos.getX(), logicalMousePos.getY(), h.getPos());
            ...
            case RP_EDIT_END:
              {
                Point2D.Double d =
                    projectPoint(logicalMousePos.getX(), logicalMousePos.getY(), h.getExtent());
