...
public class TemplateEditor extends JFrame
    implements KeyListener, ActionListener, DropTargetListener {
  ...
  */
  ...
  private void init() {
    ...
    */
    ...
    jp =
        new JScrollPane(
            _sk, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
    p.add(jp);
    p.add(_vp);
    ...
    if (e.getActionCommand().equals("undo")) {
      ...
      try {
        ...
        DrawRNATemplateCurveMethod curveMethod =
            (DrawRNATemplateCurveMethod) ellipseMethodList.getSelectedItem();
        RNATemplateMapping map =
            _vp.getRNA()
                .drawRNATemplate(
                    _sk.getTemplate(), _vp.getConfig(), method, curveMethod, getStraightBulges());
        ...
        JOptionPane.showMessageDialog(
            this, e1.getMessage(), "Template-based RNA drawing error", JOptionPane.ERROR_MESSAGE);
      }
  ...
  public void retrieveTemplates() {
    ...
    try {
      u =
          new URL(
              "http://127.0.0.1/VARNA/templateShare/actions.php?action=retrieve&nbHelices=3&nbMultiLoops=1&length=50");
