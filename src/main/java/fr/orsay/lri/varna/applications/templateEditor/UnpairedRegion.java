...
public class UnpairedRegion extends GraphicalTemplateElement {
  ...
  /**
   * Build an UnpairedRegion object from a RNATemplateUnpairedSequence object. The
   * RNATemplateUnpairedSequence must be connected to an helix on both sides.
   */
  ...
  public void draw(Graphics2D g2d, boolean selected) {
    ...
    if (!isAnchored5() && !isAnchored3()) drawMove(g2d, getCenter());
  }
  ...
  public Point2D.Double getControl5() {
    ...
    return new Point2D.Double(
        p5.x + Math.cos(angle) * _e.getInTangentVectorLength(),
  ...
  public Point2D.Double getControl3() {
    ...
    return new Point2D.Double(
        p3.x + Math.cos(angle) * _e.getOutTangentVectorLength(),
  ...
  public RelativePosition getClosestEdge(double x, double y) {
    ...
    ArrayList<Couple<java.lang.Double, RelativePosition>> v =
        new ArrayList<Couple<java.lang.Double, RelativePosition>>();
    v.add(
        new Couple<java.lang.Double, RelativePosition>(
            p.distance(p5), RelativePosition.RP_CONNECT_START5));
    v.add(
        new Couple<java.lang.Double, RelativePosition>(
            p.distance(p3), RelativePosition.RP_CONNECT_END3));
    v.add(
        new Couple<java.lang.Double, RelativePosition>(
            p.distance(t5), RelativePosition.RP_EDIT_TANGENT_5));
    v.add(
        new Couple<java.lang.Double, RelativePosition>(
            p.distance(t3), RelativePosition.RP_EDIT_TANGENT_3));
    v.add(
        new Couple<java.lang.Double, RelativePosition>(
            p.distance(ct), RelativePosition.RP_INNER_MOVE));
  ...
  public RelativePosition getRelativePosition(double x, double y) {
    ...
    if (d < MAX_UNPAIRED_CONTROL_DISTANCE) return rp;
    if (getCurve().contains(new Point2D.Double(x, y))) return RelativePosition.RP_INNER_GENERAL;
    return RelativePosition.RP_OUTER;
  }
  ...
  public void detach(RelativePosition edge) {
    ...
    if (getEndPoint(edge).isConnected()) {
      Couple<GraphicalTemplateElement.RelativePosition, GraphicalTemplateElement> c =
          getAttachedElement(edge);
