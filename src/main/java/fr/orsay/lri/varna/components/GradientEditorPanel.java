...
public class GradientEditorPanel extends JPanel implements MouseListener, MouseMotionListener {
  ...
  public void paintComponent(Graphics g) {
    ...
    for (int i = 0; i < _mcm.getNumColors(); i++) {
      ...
      if (i == 0) {
        ...
        g2d.fillRect(
            x - PALETTE_HEIGHT / 2 + 1, height + GAP + 1, PALETTE_HEIGHT - 1, PALETTE_HEIGHT - 1);
        g2d.setColor(c);
        g2d.fillOval(
            x - PALETTE_HEIGHT / 2 + 1, height + GAP + 1, PALETTE_HEIGHT - 1, PALETTE_HEIGHT - 1);
        g2d.setColor(alterColor(c, 10));
        g2d.fillOval(
            x - PALETTE_HEIGHT / 2 + 1 + 2,
            height + GAP + 1 + 2,
            PALETTE_HEIGHT - 1 - 4,
            PALETTE_HEIGHT - 1 - 4);
        ...
        g2d.drawRect(
            x - PALETTE_HEIGHT / 2,
            height + 2 * GAP + PALETTE_HEIGHT,
            REMOVE_HEIGHT,
            REMOVE_HEIGHT);
        g2d.setColor(BUTTONS);
        g2d.fillRect(
            x - PALETTE_HEIGHT / 2 + 1,
            height + 2 * GAP + 1 + PALETTE_HEIGHT,
            REMOVE_HEIGHT - 1,
            REMOVE_HEIGHT - 1);
  ...
  private boolean isChooseColor(int x, int y) {
    if (_selectedIndex != -1) {
      if ((_selectedIndex == 0) || (_selectedIndex == _mcm.getNumColors() - 1))
        return (y <= getEndRemove()
            && y >= getStartChoose()
            && Math.abs(getXPos(_selectedIndex) - x) <= PALETTE_HEIGHT);
  ...
  private boolean isRemove(int x, int y) {
    if (_selectedIndex != -1) {
      if ((_selectedIndex == 0) || (_selectedIndex == _mcm.getNumColors() - 1)) return false;
  ...
  public void mouseClicked(MouseEvent arg0) {
    ...
    if (_selectedIndex != -1) {
      if (isRemove(arg0.getX(), arg0.getY())) {
        ...
        double val =
            _mcm.getMinValue()
                + arg0.getX() * (_mcm.getMaxValue() - _mcm.getMinValue()) / (getWidth() - 1);
        Color nc =
            JColorChooser.showDialog(this, "Choose new color", _mcm.getColorAt(_selectedIndex));
        if (nc != null) {
  ...
  public void mouseDragged(MouseEvent arg0) {
    if ((_selectedIndex != -1)
        && (_selectedIndex != 0)
        && (_selectedIndex != _mcm.getNumColors() - 1)) {
