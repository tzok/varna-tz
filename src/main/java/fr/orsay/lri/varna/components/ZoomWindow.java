...
public class ZoomWindow extends JPanel
    implements ImageObserver, Runnable, MouseMotionListener, MouseListener {
  ...
  public synchronized void drawPanel() {
    if (getWidth() > 0 && getHeight() > 0) {
      ...
      rnaRect =
          _vp.renderRNA(g2D, new Rectangle2D.Double(0, 0, getWidth(), getHeight()), false, true);

      Point2D.Double p1 = _vp.panelToLogicPoint(new Point2D.Double(0., 0.));
      Point2D.Double p2 =
          _vp.panelToLogicPoint(new Point2D.Double(_vp.getWidth(), _vp.getHeight()));
      ...
      Rectangle2D.Double rvisible =
          new Rectangle2D.Double(
              rnaRect.x + rnaRect.width * (double) (p1.x - rnaBox.x) / (double) rnaBox.width,
      ...
      g2D.drawLine(
          getWidth(), getHeight(), rvisible.x + rvisible.width, rvisible.y + rvisible.height);
