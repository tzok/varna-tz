...
public class ControleurBaseSpecialColorEditor implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    if (BaseSpecialColorEditor.getEDIT().equals(e.getActionCommand())) {
      ...
      // bring up the dialog.
      _specialColorEditor.getButton().setBackground(_specialColorEditor.getCurrentColor());
      ...
      _specialColorEditor.setCurrentColor(_specialColorEditor.getColorChooser().getColor());

      _selectedRow = _specialColorEditor.get_vueBases().getTable().getSelectedRow();

      _selectedCol = _specialColorEditor.get_vueBases().getTable().getSelectedColumn();

      _selectedColor = _specialColorEditor.getCurrentColor();

      _selectedColTitle =
          _specialColorEditor.get_vueBases().getSpecialTableModel().getColumnName(_selectedCol);
  ...
  private void applyColor(String titreCol, Color couleur, ModeleBase mb) {
    if (titreCol.equals("Inner Color")) {
      mb.getStyleBase().setBaseInnerColor(couleur);
    } else if (titreCol.equals("Outline Color")) {
      mb.getStyleBase().setBaseOutlineColor(couleur);
    } else if (titreCol.equals("Name Color")) {
      mb.getStyleBase().setBaseNameColor(couleur);
    } else if (titreCol.equals("Number Color")) {
      mb.getStyleBase().setBaseNumberColor(couleur);
    }
  }
}
