...
public class ControleurClicMovement
  ...
  public void mousePressed(MouseEvent arg0) {
    ...
    if (button1 && !ctrl && !alt && !shift) {
      ...
      if ((_vp.getRealCoords() != null) && _vp.getRNA().get_listeBases().size() != 0) {
        ...
        addMenu(arg0);
        if (_vp.get_selectedAnnotation() != null) _vp.highlightSelectedAnnotation();
      }
  ...
  public void mouseDragged(MouseEvent me) {
    if ((_currentState == MouseStates.MOVE_OR_SELECT_ELEMENT)
  ...
  private void addMenu(MouseEvent arg0) {
    ...
    if (_vp.getRNA().get_listeBases().get(_vp.getNearestBase()).getElementStructure() != -1) {
      addMenuBasePair();
    }
  ...
  private void detectAnnotation(MouseEvent arg0) {
    if (_vp.getListeAnnotations().size() != 0) {
      ...
      for (TextAnnotation textAnnot : _vp.getListeAnnotations()) {
        ...
        d2 =
            Math.sqrt(
                Math.pow((position.x - arg0.getX()), 2) + Math.pow((position.y - arg0.getY()), 2));
  ...
  private void detectBulge() {
    ...
    if ((indices.size() > 0) && (_vp.getRNA().getHelixCountOnLoop(_vp.getNearestBase()) == 2)) {
      JMenu submenuBulge = new JMenu("Bulge");
      submenuBulge.addChangeListener(
          new ControleurSelectionHighlight(new Vector<Integer>(indices), _vp, submenuBulge));
      submenuBulge.setActionCommand("bulge");
      if (!_vp.isModifiable()) submenuBulge.setEnabled(false);
  ...
  private void detectHelix() {
    ...
    if (indices.size() != 0) {
      ...
      submenuHelix.addChangeListener(
          new ControleurSelectionHighlight(new Vector<Integer>(indices), _vp, submenuHelix));
      submenuHelix.setActionCommand("helix");
      if (!_vp.isModifiable()) submenuHelix.setEnabled(false);
  ...
  private void detectStem() {
    ...
    if (indices.size() > 0) {
      ...
      submenuStem.addChangeListener(
          new ControleurSelectionHighlight(new Vector<Integer>(indices), _vp, submenuStem));
      submenuStem.setActionCommand("stem");
      if (!_vp.isModifiable()) submenuStem.setEnabled(false);
  ...
  private void detect3Prime() {
    ...
    if (indices.size() != 0) {
      ...
      submenu3Prime.addChangeListener(
          new ControleurSelectionHighlight(new Vector<Integer>(indices), _vp, submenu3Prime));
      submenu3Prime.setActionCommand("3'");
      if (!_vp.isModifiable()) submenu3Prime.setEnabled(false);
  ...
  private void detect5Prime() {
    ...
    if (indices.size() != 0) {
      ...
      submenu5Prime.addChangeListener(
          new ControleurSelectionHighlight(new Vector<Integer>(indices), _vp, submenu5Prime));
      submenu5Prime.setActionCommand("5'");
      if (!_vp.isModifiable()) submenu5Prime.setEnabled(false);
  ...
  private void detectLoop() {
    ...
    if (_vp.getRNA().get_listeBases().get(indexL).getElementStructure() == -1) {
      ...
      submenuLoop.addChangeListener(new ControleurSelectionHighlight(listLoop, _vp, submenuLoop));
      submenuLoop.setActionCommand("loop1");
      if (!_vp.isModifiable()) submenuLoop.setEnabled(false);
      ...
      if (listLoop1.size() > 0) {
        ...
        if (!_vp.isModifiable()) submenuLoop1.setEnabled(false);
      ...
      if (listLoop2.size() > 0) {
        ...
        if (!_vp.isModifiable()) submenuLoop2.setEnabled(false);
  ...
  private void addCurrent() {
    ...
    if (mbs.size() > 0) {
      ...
      submenuAll.addChangeListener(new ControleurSelectionHighlight(mbs, _vp, submenuAll));
      submenuAll.setActionCommand("current");
      if (!_vp.isModifiable()) submenuAll.setEnabled(false);
  ...
  private void addMenuBase() {
    ...
    if (mb instanceof ModeleBasesComparison) {
      submenuBase.setText(
          "Base #" + (mb.getBaseNumber()) + ":" + ((ModeleBasesComparison) mb).getBases());
    } else {
      submenuBase.setText(
          "Base #" + (mb.getBaseNumber()) + ":" + ((ModeleBaseNucleotide) mb).getBase());
    }
    ...
    if (!_vp.isModifiable()) submenuBase.setEnabled(false);
  ...
  private void addAllBase() {
    ...
    submenuAll.addChangeListener(
        new ControleurSelectionHighlight(new Vector<Integer>(indices), _vp, submenuAll));
  ...
  private void addMenuBasePair() {
    ...
    if (base.getElementStructure() != -1) {
      ...
      ModeleBase partner = _vp.getRNA().get_listeBases().get(base.getElementStructure());
      submenuBasePair.addChangeListener(
          new ControleurSelectionHighlight(indices, _vp, submenuBasePair));
      submenuBasePair.setText(
          "Base pair #("
              + (Math.min(base.getBaseNumber(), partner.getBaseNumber()))
              + ","
              + (Math.max(base.getBaseNumber(), partner.getBaseNumber()))
              + ")");
      submenuBasePair.setActionCommand("bp");
      // option disponible seulement en mode modifiable
      if (!_vp.isModifiable()) submenuBasePair.setEnabled(false);
