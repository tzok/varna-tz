...
/**
 * It controls the sequence and structure text fields and changes their color if they have different
 * lengths or unbalanced parentheses.
 *
 * @author darty
 */
public class ControleurDemoTextField implements CaretListener {
  ...
  public void caretUpdate(CaretEvent e) {
    if (_oldStruct != _vod.get_struct().getText() || _oldSeq != _vod.get_seq().getText()) {
      ...
      if (pb || p.size() > 0) {
        ...
        if (_vod.get_seq().getText().length() != _vod.get_struct().getText().length()) {
          // colorie en orange
