...
public class ControleurInterpolator extends Thread {
  ...
  public synchronized void addTarget(RNA target) {
    addTarget(
        target,
        null,
        Mapping.DefaultOutermostMapping(
            _vpn.getRNA().get_listeBases().size(), target.get_listeBases().size()));
  }
  ...
  private Point2D.Double computeDestination(
      Point2D pli, Point2D pri, Point2D pi, int i, int n, Point2D plf, Point2D prf) {
    Point2D.Double plm =
        new Point2D.Double((pli.getX() + plf.getX()) / 2.0, (pli.getY() + plf.getY()) / 2.0);
    Point2D.Double prm =
        new Point2D.Double((pri.getX() + prf.getX()) / 2.0, (pri.getY() + prf.getY()) / 2.0);
    Point2D.Double pm =
        new Point2D.Double(
            ((n - i) * plm.getX() + i * prm.getX()) / n,
            ((n - i) * plm.getY() + i * prm.getY()) / n);
  ...
  private Vector<Vector<Integer>> clusterIndices(int numIndices, int[] mappedIndices)
  ...
  /**
   * Builds and returns an interval array, alternating unmatched regions and matched indices.
   * Namely, returns a vector of vector such that the vectors found at odd positions contain those
   * indices that are NOT associated with any other base in the current mapping. On the other hand,
   * vectors found at even positions contain only one mapped index. <br>
   * Ex: If indices=<code>[1,2,3,4,5]</code> and mappedIndices= <code>[2,5]</code> then the function
   * will return <code>[[1],[2],[3,4],[5],[]]</code>.
   *
   * @param indices The total list of indices
   * @param mappedIndices Matched indices, should be a subset of <code>indices</code>
   * @return A clustered array
   * @throws MappingException If one of the parameters is an empty array
   */
  private Vector<Vector<Integer>> clusterIndices(int[] indices, int[] mappedIndices)
      throws MappingException {
  ...
  /** Compute the centroid of the RNA bases that have their indexes in the given array. */
  ...
  /**
   * The purpose of this class is to compute the rotation that minimizes the RMSD between the bases
   * of the first RNA and the bases of the second RNA.
   *
  ...
  private static class MinimizeRMSD {
    ...
    /**
     * A function such that minimizing it is equivalent to minimize the RMSD between between the two
     * arrays of vectors, supposing we rotate the points in [X2,Y2] by theta.
     */
    ...
    /** Find the theta that minimizes f(theta). We use Newton's method. */
  ...
  /**
   * We suppose we have two lists of points. The coordinates of the first list of points are X1 and
   * Y1, and X2 and Y2 for the second list. We suppose that the centroids of [X1,Y1] and [X2,Y2] are
   * both (0,0). This function computes the rotation to apply to the second set of points such that
   * the RMSD (Root mean square deviation) between them is minimum. The returned angle is in
   * radians.
   */
  ...
  /**
   * Move rna2 using a rotation so that it would move as little as possible (in the sense that we
   * minimize the RMSD) when transforming it to rna1 using the given mapping.
   */
  ...
  /**
   * The argument moveTarget specifies whether the RNA _target should be rotated so that bases move
   * as little as possible when switching from the current RNA to _target using the animation. Note
   * that this will modify the _target object directly.
   */
  public void nextTarget(RNA _target, VARNAConfig _conf, Mapping _mapping, boolean moveTarget) {
    try {
      ...
      if (source.getSize() != 0 && _target.getSize() != 0) {
        ...
        intArrSource = clusterIndices(currBases.size(), _mapping.getSourceElems());
        intArrTarget = clusterIndices(destBases.size(), _mapping.getTargetElems());
        ...
        for (int i = 0; i < finalPosSource.length; i++) {
          if (_mapping.getPartner(i) != Mapping.UNKNOWN) {
            ...
            finalPosSource[i] = new Point2D.Double(dest.getX(), dest.getY());
          }
        }
        ...
        for (int i = 0; i < intArrSource.size(); i += 2) {
          ...
          if (i == 0) {
            ...
            matchedNeighborLeft = intArrSource.get(intArrSource.size() - 2).get(0);
            matchedNeighborRight = intArrSource.get(intArrSource.size() - 2).get(0);
          } else {
        ...
        for (int i = 0; i < intArrTarget.size(); i += 2) {
          ...
          if (i == 0) {
            ...
            matchedNeighborLeft = intArrTarget.get(intArrTarget.size() - 2).get(0);
            matchedNeighborRight = intArrTarget.get(intArrTarget.size() - 2).get(0);
          } else {
        ...
        for (int i = 0; i < _numSteps; i++) {
          ...
          for (int j = 0; j < currBases.size(); j++) {
            ...
            m.setCoords(
                new Point2D.Double(
                    ((_numSteps - 1 - i) * mpc.getX() + (i) * mnc.getX()) / (_numSteps - 1),
                    ((_numSteps - 1 - i) * mpc.getY() + i * mnc.getY()) / (_numSteps - 1)));
          }
          _vpn.repaint();
          sleep(_timeDelay);
        }
