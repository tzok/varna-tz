 ...
 * @author darty
 */
public class ControleurMenu implements InterfaceVARNAListener, ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    _source = e.getSource();
    _type = temp[0];
    if (temp.length > 1) _color = temp[1];
    else _color = "";
                              ...
                              if (!optionStem())
                                if (!optionBulge())
                                  if (!optionAnnotation())
                                    if (!optionEditRNA())
                                      _vp.errorDialog(
                                          new Exception("Uknown action command '" + _type + "'"));
  }
  ...
  private TextAnnotation.AnchorType trouverAncrage() {
    if (_type.contains("loop")) return TextAnnotation.AnchorType.LOOP;
    if (_type.contains("helix")) return TextAnnotation.AnchorType.HELIX;
    if (_type.contains("base")) return TextAnnotation.AnchorType.BASE;
  ...
  /**
   * Return the extension of a file, it means the string after the last dot of the file name
   *
   * @param f The file
  ...
  /**
   * Return the extension of a string, it means the string after the last dot of the path
   *
   * @param s The strnig o the path
