...
public class ControleurScriptParser {
  ...
  private enum Function {
    ...
    SET_RNA_SMOOTH(
        "setrnasmooth", new ArgumentType[] {ArgumentType.STRING_TYPE, ArgumentType.STRING_TYPE}),
  ...
  private static Hashtable<Function, ArgumentType[]> _fun2Prot =
      new Hashtable<Function, ArgumentType[]>();
  ...
  private static Function getFunction(String f) {
    ...
    if (_name2Fun.containsKey(s)) return _name2Fun.get(s);
    return Function.UNKNOWN;
  }

  private static ArgumentType[] getPrototype(Function f) {
    if (_fun2Prot.containsKey(f)) return _fun2Prot.get(f);
        ...
        case ADD_CHEM_PROB:
          {
            ...
            ChemProbAnnotationType t =
                ChemProbAnnotation.annotTypeFromString(
            ...
            for (int j = 0; j < arg.getSize(); j++) {
              ...
              if (a._t == ArgumentType.ARRAY_TYPE) {
                ...
                if (aarg.getSize() == 2) {
                  ...
                  // System.out.println("& |"+a1+"| ["+a1.getType()+"] |"+a2+"|
                  // ["+a2.getType()+"]");
                  if ((a1.getType() == ArgumentType.NUMBER_TYPE)
                      && (a2.getType() == ArgumentType.COLOR_TYPE)) {
                    // System.out.println("+");
                    cm.addColor(
                        ((NumberArgument) a1).getNumber().doubleValue(),
        ...
        case REDRAW:
          {
            ...
            if (modeStr.equals("radiate")) mode = RNA.DRAW_MODE_RADIATE;
            else if (modeStr.equals("circular")) mode = RNA.DRAW_MODE_CIRCULAR;
            else if (modeStr.equals("naview")) mode = RNA.DRAW_MODE_NAVIEW;
            else if (modeStr.equals("linear")) mode = RNA.DRAW_MODE_LINEAR;
            if (mode != -1) vp.drawRNA(vp.getRNA(), mode);
          }
          break;
  ...
  private static Boolean parseBoolean(String s) {
    Boolean result = null;
    if (s.toLowerCase().equals("true")) result = new Boolean(true);
    if (s.toLowerCase().equals("false")) result = new Boolean(false);
    return result;
  }

  private static Vector<Argument> parseArguments(StreamTokenizer st, boolean parType)
      throws Exception {
  ...
  private static boolean checkArgs(Function f, Vector<Argument> argv) throws Exception {
    ...
    if (argtypes.length != argv.size())
      throw new Exception(
          SCRIPT_ERROR_PREFIX + ": Wrong number of argument for function \"" + f + "\".");
