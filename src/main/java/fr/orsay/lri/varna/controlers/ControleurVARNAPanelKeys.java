...
public class ControleurVARNAPanelKeys implements KeyListener, FocusListener {
   ...
   * _vp.getVARNAUI().UIZoomIn(); } else if (e.getKeyCode() == KeyEvent.VK_MINUS) {
   * _vp.getVARNAUI().UIZoomOut(); } // 1 pour Redraw Radiate else if (e.getKeyChar() ==
   * KeyEvent.VK_1) { _vp.getVARNAUI().UIRadiate(); } // 2 pour Redraw Circular else if
   * (e.getKeyChar() == KeyEvent.VK_2) { _vp.getVARNAUI().UICircular(); } // 3 pour Redraw NAView
   * else if (e.getKeyChar() == KeyEvent.VK_3) { _vp.getVARNAUI().UINAView(); }
   *
   * <p>// 4 for RNA on a line else if (e.getKeyChar() == KeyEvent.VK_4) {
   * _vp.getVARNAUI().UILine(); } // 5 fun arn random coord else if (e.isControlDown() &&
   * e.getKeyChar() == KeyEvent.VK_9) { for (int i = 0; i < _vp.getRNA().get_listeBases().size();
   * i++) { _vp.getRNA().get_listeBases().get(i).set_coords( new Point2D.Double(_vp.getWidth() *
   * Math.random(), _vp.getHeight() * Math.random()));
   * _vp.getRNA().get_listeBases().get(i).set_center( new Point2D.Double(_vp.getWidth() / 2
   * Math.random(), _vp.getHeight() / 2 Math.random())); } } // 6 fun random arn structure else if
   * (e.isControlDown() & e.getKeyChar() == KeyEvent.VK_8) { try {
   * _vp.drawRNA(_vp.getRNA().getListeBasesToString(), getRandomRNA(), _vp
   * .getRNA().get_drawMode()); } catch (ExceptionNonEqualLength e1) { _vp.errorDialog(e1); } }
   * _vp.repaint(); }
   */
