...
public class VARNAConfigLoader {
  ...
  public static String[][] getParameterInfo() {
    String[][] info = {
      ...
      {applyBasesStyleOpt, "String", "Base style application"},
      {
        auxBPsOpt,
        "String",
        "Adds a list of (possibly non-canonical) base-pairs to those already defined by the main"
            + " secondary structure (Ex: \"(1,10);(2,11);(3,12)\"). Custom BP styles can be"
            + " specified (Ex: \"(2,11):thickness=4;(3,12):color=#FF0000\")."
      },
      {autoHelicesOpt, "", ""},
      ...
      {chemProbOpt, "", ""},
      {
        colorMapOpt,
        "String",
        "Associates a list of numerical values (eg '0.2,0.4,0.6,0.8') with the RNA bases with"
            + " respect to their natural order, and modifies the color used to fill these bases"
            + " according to current color map style."
      },
      {colorMapCaptionOpt, "String", "Sets current color map caption."},
      {
        colorMapDefOpt,
        "String",
        "Selects a specific color map style. It can be either one of the predefined styles (eg"
            + " 'red', 'green', 'blue', 'bw', 'heat', 'energy') or a new one (eg"
            + " '0:#FFFF00;1:#ffFFFF;6:#FF0000')."
      },
      {colorMapMinOpt, "", ""},
      {colorMapMaxOpt, "", ""},
      ...
      {fillBasesOpt, "boolean", "Fills or leaves empty the inner portions of bases"},
      ...
      },
      {
        flipOpt,
        "String",
        "Draws a set of exterior helices, identified by the argument string, in clockwise order"
            + " (default drawing is counter-clockwise). The argument is a semicolon-separated list"
            + " of helices, each identified by a base or a base-pair (eg. \"2;20-34\")."
      },
      ...
      {numRowsOpt, "int", "Sets number of rows"},
      {
        orientationOpt,
        "float",
        "Sets the general orientation of an RNA, i.e. the deviation of the longest axis (defined by"
            + " the most distant couple of bases) from the horizontal axis."
      },
      ...
      {spaceBetweenBasesOpt, "float", "Sets the space between consecutive bases"},
  ...
  private void retrieveParametersValues() throws ExceptionParameterError {
    ...
    try {
      _numRows = Integer.parseInt(_optionProducer.getParameterValue(numRowsOpt, "" + _numRows));
    } catch (NumberFormatException e) {
      throw new ExceptionParameterError(
          e.getMessage(),
          "'"
              + _optionProducer.getParameterValue(numRowsOpt, "" + _numRows)
              + "' is not a integer value for the number of rows !");
    }
    try {
      _numColumns =
          Integer.parseInt(_optionProducer.getParameterValue(numColumnsOpt, "" + _numColumns));
    } catch (NumberFormatException e) {
      throw new ExceptionParameterError(
          e.getMessage(),
          "'"
              + _optionProducer.getParameterValue(numColumnsOpt, "" + _numColumns)
      ...
      for (int j = 0; j < _numRows; j++) {
        try {
          ...
          try {
            _rotation =
                Double.parseDouble(
                    _optionProducer.getParameterValue(rotationOpt + n, Double.toString(_rotation)));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(
                e.getMessage(),
                "'"
                    + _optionProducer.getParameterValue(rotationOpt + n, "" + _rotation)
            ...
            _colorMapMin =
                Double.parseDouble(
                    _optionProducer.getParameterValue(
                        colorMapMinOpt + n, Double.toString(this._colorMapMin)));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(
                e.getMessage(),
                "'"
                    + _optionProducer.getParameterValue(colorMapMinOpt + n, "" + _colorMapMin)
          ...
          try {
            _colorMapMax =
                Double.parseDouble(
                    _optionProducer.getParameterValue(
                        colorMapMaxOpt + n, Double.toString(this._colorMapMax)));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(
                e.getMessage(),
                "'"
                    + _optionProducer.getParameterValue(colorMapMaxOpt + n, "" + _colorMapMax)
          ...
          try {
            _bpIncrement =
                Double.parseDouble(
                    _optionProducer.getParameterValue(
                        bpIncrementOpt + n, Double.toString(_bpIncrement)));
          } catch (NumberFormatException e) {
          }
          ...
          try {
            _periodResNum =
                Integer.parseInt(
                    _optionProducer.getParameterValue(periodNumOpt + n, "" + _periodResNum));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(
                e.getMessage(),
                "'"
          ...
          try {
            _titleSize =
                Integer.parseInt(
                    _optionProducer.getParameterValue(titleSizeOpt + n, "" + _titleSize));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(
                e.getMessage(),
                "'"
          ...
          try {
            _zoom = Double.parseDouble(_optionProducer.getParameterValue(zoomOpt + n, "" + _zoom));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(
                e.getMessage(),
                "'"
                    + _optionProducer.getParameterValue(zoomOpt + n, "" + _zoom)
          ...
          try {
            _zoomAmount =
                Double.parseDouble(
                    _optionProducer.getParameterValue(zoomAmountOpt + n, "" + _zoomAmount));
          } catch (NumberFormatException e) {
            throw new ExceptionParameterError(
                e.getMessage(),
                "'"
          ...
          _drawBases =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(drawBasesOpt + n, "" + _drawBases));
          _fillBases =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(fillBasesOpt + n, "" + _fillBases));
          _autoHelices =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(autoHelicesOpt + n, "" + _autoHelices));
          _drawColorMap =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(drawColorMapOpt + n, "" + _drawColorMap));
          _drawBackbone =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(drawBackboneOpt + n, "" + _drawBackbone));
          _colorMapValues = _optionProducer.getParameterValue(colorMapOpt + n, _colorMapValues);
          _autoTerminalLoops =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(
                      autoTerminalLoopsOpt + n, "" + _autoTerminalLoops));
          _autoInteriorLoops =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(
                      autoInteriorLoopsOpt + n, "" + _autoInteriorLoops));
          _drawNC =
              Boolean.parseBoolean(_optionProducer.getParameterValue(drawNCOpt + n, "" + _drawNC));
          _flatExteriorLoop =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(
                      flatExteriorLoopOpt + n, "" + _flatExteriorLoop));
          _drawTertiary =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(drawTertiaryOpt + n, "" + _drawTertiary));
          _warning =
              Boolean.parseBoolean(_optionProducer.getParameterValue(warningOpt + n, "false"));
          ...
          _comparisonMode =
              Boolean.parseBoolean(
                  _optionProducer.getParameterValue(comparisonModeOpt + n, "false"));
          _firstSequence =
              _optionProducer.getParameterValue(firstSequenceForComparisonOpt + n, _firstSequence);
          _firstStructure =
              _optionProducer.getParameterValue(
          ...
          _highlightRegion =
              _optionProducer.getParameterValue(highlightRegionOpt + n, _highlightRegion);
          ...
          _colorMapCaption =
              _optionProducer.getParameterValue(colorMapCaptionOpt + n, _colorMapCaption);
          _backboneColor =
              getSafeColor(
                  _optionProducer.getParameterValue(
                      backboneColorOpt + n, _backboneColor.toString()),
                  _backboneColor);
          _backgroundColor =
              getSafeColor(
                  _optionProducer.getParameterValue(
                      backgroundColorOpt + n, _backgroundColor.toString()),
                  _backgroundColor);
          _bondColor =
              getSafeColor(
                  _optionProducer.getParameterValue(bondColorOpt + n, _bondColor.toString()),
                  _bondColor);
          _bpStyle =
              getSafeBPStyle(_optionProducer.getParameterValue(bpStyleOpt + n, ""), _bpStyle);
          _flip = _optionProducer.getParameterValue(flipOpt + n, _flip);
  ...
  private void applyValues(String n)
    ...
    else if (_algo.equals(ALGORITHM_NAVIEW)) algoCode = RNA.DRAW_MODE_NAVIEW;
    else if (_algo.equals(ALGORITHM_LINE)) algoCode = RNA.DRAW_MODE_LINEAR;
    else if (_algo.equals(ALGORITHM_RADIATE)) algoCode = RNA.DRAW_MODE_RADIATE;
    else if (_algo.equals(ALGORITHM_VARNA_VIEW)) algoCode = RNA.DRAW_MODE_VARNA_VIEW;
    else if (_algo.equals(ALGORITHM_MOTIF_VIEW)) algoCode = RNA.DRAW_MODE_MOTIFVIEW;
    else algoCode = RNA.DRAW_MODE_RADIATE;

    if (_comparisonMode) {
      _mainSurface =
          new VARNAPanel(
              _firstSequence, _firstStructure, _secondSequence, _secondStructure, algoCode, "");
    } else {
    ...
    if (!_URL.equals("")) {
      ...
      try {
        ...
        if (_URL.toLowerCase().endsWith(VARNAPanel.VARNA_SESSION_EXTENSION)) {
          ...
          Collection<RNA> rnas =
              RNAFactory.loadSecStr(
                  new BufferedReader(inr), RNAFactory.guessFileTypeFromExtension(_URL));
          if (rnas.isEmpty()) {
        ...
        if (url != null) e.setPath(url.getPath());
            ...
            try {
              ...
              if (args.length == 1) {
                ...
                if ((indexFrom != -1) && (indexTo != -1)) {
                  for (int l = indexFrom; l <= indexTo; l++) indicesList.add(l);
                }
              }
            } catch (NumberFormatException e) {
  ...
  private void applyHighlightRegion(VARNAPanel vp) {
    ...
    for (int i = 0; i < regions.length; i++) {
      ...
      try {
        HighlightRegionAnnotation nt =
            HighlightRegionAnnotation.parseHighlightRegionAnnotation(region, vp);
        if (nt != null) {
        ...
        if (data.length >= 1) {
          ...
          if (data2.length == 2) {
            ...
            if (s1.startsWith("(") && s2.endsWith(")")) {
              ...
              ModeleBP msbp =
                  new ModeleBP(
                      vp.getRNA().get_listeBases().get(c), vp.getRNA().get_listeBases().get(d));
              if (data.length >= 2) {
                String style = data[1];
                msbp.assignParameters(style);
              }
