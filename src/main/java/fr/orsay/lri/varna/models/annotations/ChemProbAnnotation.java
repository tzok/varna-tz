...
public class ChemProbAnnotation implements Serializable {
  ...
  public ChemProbAnnotation(
      ModeleBase mbfst, ModeleBase mbsnd, ChemProbAnnotationType type, double intensity) {
  ...
  public Point2D.Double getDirVector() {
    ...
    Point2D.Double vradius =
        new Point2D.Double((center.x - anchor.x) / 2.0, (center.y - anchor.y) / 2.0);
    if (_outward) {
  ...
  public String toString() {
    return "Chem. prob. "
        + this._type
        + " Base#"
        + this._mbfst.getBaseNumber()
        + "-"
        + this._mbsnd.getBaseNumber();
  }
}
