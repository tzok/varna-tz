...
public class HighlightRegionAnnotation implements Serializable {
  ...
  public void toXML(TransformerHandler hd) throws SAXException {
    ...
    atts.addAttribute(
        "", "", XML_VAR_OUTLINE_NAME, "CDATA", "" + XMLUtils.toHTMLNotation(_outlineColor));
  ...
  public HighlightRegionAnnotation(
      ArrayList<ModeleBase> b, Color fill, Color outline, double radius) {
    _bases = b;
    _fillColor = fill;
  ...
  private LinkedList<Point2D.Double> buildRoundedCorner(
      Point2D.Double p1, Point2D.Double p2, Point2D.Double anotherPoint) {
    ...
    Point2D.Double pdir =
        new Point2D.Double(
            m.x + rad * Math.cos(angle + Math.PI / 2.), m.y + rad * Math.sin(angle + Math.PI / 2.));
    ...
    for (int k = 1; k <= NUM_STEPS_ROUNDED_CORNERS; k++) {
      ...
      Point2D.Double interForward =
          new Point2D.Double(m.x + rad * Math.cos(angle2), m.y + rad * Math.sin(angle2));
  ...
  public GeneralPath getShape(
      Point2D.Double[] realCoords, Point2D.Double[] realCenters, double scaleFactor) {
    ...
    for (int i = 0; i < getBases().size(); i++) {
      int j1 = getBases().get(i).getIndex();
      {
        ...
        double h =
            (new Point2D.Double(vn2.x - vn1.x, vn2.y - vn1.y).distance(new Point2D.Double(0, 0))
                / 2.0);
        ...
        Point2D.Double interForward =
            new Point2D.Double(
                p1.x + nnormF * scaleFactor * getRadius() * vn.x,
                p1.y + nnormF * scaleFactor * getRadius() * vn.y);
        Point2D.Double interBackward =
            new Point2D.Double(
                p1.x - nnormB * scaleFactor * getRadius() * vn.x,
        ...
        if (pointList.size() > 0) {
          ...
          if ((interForward.distance(prev1) + interBackward.distance(prev2))
              < (interForward.distance(prev2) + interBackward.distance(prev1))) {
    ...
    if (getBases().size() == 1) {
      ...
      LinkedList<Point2D.Double> pointListStart =
          buildRoundedCorner(pointList.get(midl - 1), pointList.get(midl), apoint);
      pointList.addAll(midl, pointListStart);
      mid = pointList.get(midl);
      apoint = new Point2D.Double(mid.x + 1., mid.y);
      LinkedList<Point2D.Double> pointListEnd =
          buildRoundedCorner(pointList.get(pointList.size() - 1), pointList.get(0), apoint);
      ...
      apoint =
          symImage(
              realCoords[getBases().get(getBases().size() - 1).getIndex()],
  ...
  public static HighlightRegionAnnotation parseHighlightRegionAnnotation(
      String txt, VARNAPanel vp) {
    try {
