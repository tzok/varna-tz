...
public class TextAnnotation implements Serializable {
  ...
  /**
   * creates an annoted text on a VARNAPanel with the specified text fixed to a helix (if type is
   * HELIX) or to a loop (if type is LOOP)
   *
   * @param texte
   * @param listeBase
   * @param type
   * @throws Exception
   */
  public TextAnnotation(String texte, ArrayList<ModeleBase> listeBase, AnchorType type)
      throws Exception {
    this(texte);
    _anchor = listeBase;

    if (type == AnchorType.HELIX) _typeAnchor = AnchorType.HELIX;
    else if (type == AnchorType.LOOP) _typeAnchor = AnchorType.LOOP;
  ...
  public String toString() {
    ...
    switch (_typeAnchor) {
      case POSITION:
        ...
        return tmp
            + " at ("
            + formatter.format(getCenterPosition().x)
            + ","
            + formatter.format(getCenterPosition().y)
            + ")";
      case BASE:
    ...
    try {
      switch (_typeAnchor) {
        case BASE:
          ...
          break;
        case POSITION:
          textAnnot = new TextAnnotation(_text, ((VARNAPoint) _anchor).x, ((VARNAPoint) _anchor).y);
          break;
        case LOOP:
          textAnnot =
              new TextAnnotation(_text, extractedArrayListModeleBaseFromAncrage(), AnchorType.LOOP);
          break;
        case HELIX:
          textAnnot =
              new TextAnnotation(
                  _text, extractedArrayListModeleBaseFromAncrage(), AnchorType.HELIX);
          break;
        default:
          break;
      }
