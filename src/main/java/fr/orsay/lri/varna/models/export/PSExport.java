...
public class PSExport extends SecStrDrawingProducer {
  ...
  private String PSArc(
      double x, double y, double radiusX, double radiusY, double angleFrom, double angleTo) {
  ...
  private String PSBBox(double minX, double maxX, double minY, double maxY) {
    String norm =
        ("%%BoundingBox: "
            + (long) Math.floor(minX)
            + " "
            + (long) Math.floor(minY)
            + " "
            + (long) Math.ceil(maxX)
            + " "
            + (long) Math.ceil(maxY)
  ...
  public String drawArcS(
    ...
    return PSArc(origine.x, origine.y, width, height, startAngle, endAngle) + PSStroke();
  }
