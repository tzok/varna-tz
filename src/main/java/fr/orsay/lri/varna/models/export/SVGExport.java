...
public class SVGExport extends SecStrDrawingProducer {
  ...
  public String drawRectangleS(Point2D.Double orig, Point2D.Double dims, double thickness) {
    ...
    return "<rect x=\""
        + orig.x
        + "\" y=\""
        + (_bb.height - orig.y - dims.y)
        + "\" width=\""
        + dims.x
        + "\" height=\""
        + dims.y
        + "\" fill=\"none\" style=\"stroke-width:"
        + thickness
        + ";stroke:"
        + getRGBString(_curColor)
        + "\"/>";
  ...
  private Point2D.Double polarToCartesian(
      Point2D.Double center, double radiusX, double radiusY, double angleInDegrees) {
    double angleInRadians = (angleInDegrees) * Math.PI / 180.0;

    return new Point2D.Double(
        center.x + (radiusX * Math.cos(angleInRadians)),
        _bb.height - (center.y + (radiusY * Math.sin(angleInRadians))));
  }

  public String drawArcS(
      Point2D.Double o, double width, double height, double startAngle, double endAngle) {
    double rx = width / 2.0;
