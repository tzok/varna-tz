...
public abstract class SecStrDrawingProducer {
  ...
  public abstract String drawLineS(Point2D.Double orig, Point2D.Double dest, double thickness);

  public abstract String drawArcS(
  ...
  public abstract String drawRectangleS(Point2D.Double orig, Point2D.Double dims, double thickness);

  public abstract String drawCircleS(Point2D.Double base, double radius, double thickness);

  public abstract String fillCircleS(
      Point2D.Double base, double radius, double thickness, Color color);

  public abstract String drawPolygonS(Point2D.Double[] points, double thickness);

  public abstract String fillPolygonS(Point2D.Double[] points, Color color);
  ...
  public void drawLine(double x0, double y0, double x1, double y1, double thickness) {
    ...
    _commands.add(
        new LineCommand(new Point2D.Double(x0, y0), new Point2D.Double(x1, y1), thickness));
  }
  ...
  public void drawArc(
    ...
    updateBoundingBox(origine.x - width / 2., origine.y - height / 2.);
    _commands.add(new ArcCommand(origine, width, height, startAngle, endAngle));
  }
  ...
  public void drawRectangle(double x, double y, double w, double h, double thickness) {
    ...
    _commands.add(
        new RectangleCommand(new Point2D.Double(x, y), new Point2D.Double(w, h), thickness));
  }
  ...
  public void drawCircle(double x, double y, double radius, double thickness) {
    ...
    _commands.add(new CircleCommand(new Point2D.Double(x, y), radius, thickness));
  }
  ...
  public void fillCircle(double x, double y, double radius, double thickness, Color color) {
    ...
    _commands.add(new FillCircleCommand(new Point2D.Double(x, y), radius, thickness, color));
  }
  ...
  public Rectangle2D.Double getBoundingBox() {
    return (new Rectangle2D.Double(_xmin, _ymin, _xmax - _xmin, _ymax - _ymin));
  }
  ...
  public String export() {
    ...
    double dx = -oldbb.x;
    double dy = -oldbb.y;
    Rectangle2D.Double nbb =
        new Rectangle2D.Double(0, 0, oldbb.width * _scale, oldbb.height * _scale);
    ...
    for (int i = 0; i < _commands.size(); i++) {
      ...
      if (ge instanceof LineCommand) {
        ...
        String tmp =
            drawLineS(
                transform(c.get_orig(), _scale, dx, dy),
                transform(c.get_dest(), _scale, dx, dy),
                c.get_thickness());
        buf.append(tmp);
      } else if (ge instanceof TextCommand) {
        TextCommand c = (TextCommand) ge;
        String tmp = drawTextS(transform(c.get_base(), _scale, dx, dy), c.get_txt());
        buf.append(tmp);
        ...
        String tmp =
            drawRectangleS(
                transform(c.get_orig(), _scale, dx, dy),
                transform(c.get_dims(), _scale, 0.0, 0.0),
                c.get_thickness());
        buf.append(tmp);
        ...
        String tmp =
            drawCircleS(
                transform(c.get_base(), _scale, dx, dy),
                c.get_radius() * _scale,
                c.get_thickness());
        buf.append(tmp);
        ...
        String tmp =
            drawArcS(
                transform(c.getCenter(), _scale, dx, dy),
                c.getWidth() * _scale,
                c.getHeight() * _scale,
                c.getStartAngle(),
                c.getEndAngle());
        ...
        String tmp = fillPolygonS(points, c.get_color());
        buf.append(tmp);
      }
    }
    buf.append(footerS());
