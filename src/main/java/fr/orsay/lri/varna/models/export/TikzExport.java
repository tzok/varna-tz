...
public class TikzExport extends SecStrDrawingProducer {
  ...
  public String drawCircleS(Point2D.Double p, double radius, double thickness) {
    return "  \\draw[draw=currColor] "
        + formatPoint(p)
        + " circle ("
        + (radius * super.getScale())
        + ");\n";
  }
  ...
  public String drawRectangleS(Point2D.Double p, Point2D.Double dims, double thickness) {
    return "  \\draw[draw=currColor] "
        + formatPoint(p)
        + " -- "
        + formatPoint(p.x + dims.x, p.y)
        + " -- "
        + formatPoint(p.x + dims.x, p.y + dims.y)
        + " -- "
        + formatPoint(p.x, p.y + dims.y)
        + " -- "
        + formatPoint(p)
        + ";\n";
  }
  ...
  public String setColorS(Color col) {
    super.setColorS(col);
    return "\\definecolor{currColor}{rgb}{"
        + (((double) col.getRed()) / 255.)
        + ","
