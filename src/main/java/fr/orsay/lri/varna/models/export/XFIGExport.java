  ...
  public String drawPolygonS(Double[] points, double thickness) {
    if (points.length > 0) {
      String result =
          "2 3 0 1 "
              + getCurColorCode()
              + " 7 40 0 -1 4.000 0 0 0 0 0 "
              + (points.length + 1)
              + "\n";
      for (int i = 0; i < points.length; i++) {
        result += (int) Math.round(points[i].x) + " " + (int) Math.round(-points[i].y) + " ";
      }
      result += (int) Math.round(points[0].x) + " " + (int) Math.round(-points[0].y) + " ";
      result += "\n";
      return result;
    } else {
      return "";
