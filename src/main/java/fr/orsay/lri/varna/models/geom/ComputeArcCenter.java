...
public class ComputeArcCenter {
  ...
  /**
   * Given an arc length (l) and segment length (delta) of the arc, find where to put the center,
   * returned as a position of the perpendicular bisector of the segment. The positive side is the
   * one where the arc is drawn. It works using Newton's method.
   */
  ...
  private static double fprime(double c, double delta) {
    if (c < 0) {
      return delta / (c * c + delta / 4) * Math.sqrt(delta * delta / 4 + c * c)
          + 2 * Math.atan(delta / (-2 * c)) * c / Math.sqrt(delta * delta / 4 + c * c);
    } else if (c != 0) { // c > 0
      return delta / (c * c + delta / 4) * Math.sqrt(delta * delta / 4 + c * c)
          + (2 * Math.PI - 2 * Math.atan(delta / (-2 * c)))
              * c
              / Math.sqrt(delta * delta / 4 + c * c);
    } else { // c == 0
      return 2;
    }
  }
