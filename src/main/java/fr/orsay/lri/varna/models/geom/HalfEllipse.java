...
/**
 * Ellipse, with axis = X and Y. This class is useful for constant speed parameterization (just like
 * CubicBezierCurve). The ellipse drawn is in fact an half-ellipse, from 0 to PI.
 *
...
public class HalfEllipse {
  ...
  /** The standard ellipse parameterization. Argument t must be in [0,1]. */
  ...
  /**
   * An ellipse that has axis equal to X and Y axis needs only two numbers (half-axis lengths) to be
   * defined. They are resp. a for X axis and b for Y axis. n = how many line segments we want to
   * cut the curve (if n is bigger the computation takes longer but the precision is better). The
  ...
  /**
   * Returns that affine transform that moves the ellipse given by this class such that its 0/pi
   * axis matches P0-P1.
   */
