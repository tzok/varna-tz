/** File written by Raphael Champeimont UMR 7238 Genomique des Microorganismes */
...
public class DrawRNATemplate {
  ...
  /**
   * Draw this RNA like the given template. The helixLengthAdjustmentMethod argument tells what to
   * do in case some helices are of a different length in the template and the actual helix. See
   * class DrawRNATemplateMethod above for possible values.
   *
  ...
  public void drawRNATemplate(
    ...
    if (helixLengthAdjustmentMethod == DrawRNATemplateMethod.MAXSCALINGFACTOR) {
      ...
      // debug
      // System.out.println("Max helix length increase factor = " + maxLengthIncreaseFactor + "
      // reached with helix " + maxIncreaseHelix);;
    ...
    while (computeCoords) {
      ...
      while (iter.hasNext()) {
        element = iter.next();
        if (element instanceof RNATemplateHelix && mapping.getAncestor(element) != null) {
          ...
          if (lastMappedHelix != null) {
            ...
            if (lastMappedHelix == helix) {
              ...
              computeHelixEndPointDirections(
                  howWeGotOutOfLastHelix, helixVector, new Point2D.Double());
              ...
              if (howWeGotOutOfLastHelix.getOtherElement() instanceof RNATemplateUnpairedSequence
                  && howWeGetInCurrentHelix.getOtherElement()
                      instanceof RNATemplateUnpairedSequence) {
            ...
            if (templateSequence != null) {
              ...
              coords[b1].setLocation(
                  coords[b2].x + j.x * idealLength, coords[b2].y + j.y * idealLength);
            }
      ...
      if (howWeGotOutOfLastHelixBaseIndex < coords.length - 1
        ...
        if (endTemplateSequence != null) {
          ...
          coords[b2].setLocation(
              coords[b1].x + j.x * idealLength, coords[b1].y + j.y * idealLength);
        }
    ...
    for (int i = 0; i < _listeBases.size(); i++) {
      _listeBases
          ...
          .setCoords(
              new Point2D.Double(
                  coords[i].x * conf._spaceBetweenBases, coords[i].y * conf._spaceBetweenBases));
      _listeBases
          .get(i)
          .setCenter(
              new Point2D.Double(
                  centers[i].x * conf._spaceBetweenBases, centers[i].y * conf._spaceBetweenBases));
    }
  }

  /**
   * IN: Argument helixEndPoint is an IN argument (will be read), and must contain an helix edge
   * endpoint.
   *
   * <p>The other arguments are OUT arguments (must be existing objects, content will be
   * overwritten).
   *
   * <p>OUT: The i argument will contain a vector colinear to the vector from the helix
   * startPosition to endPosition or the opposite depending on there the endpoint is (the endpoint
   * will be on the destination side of the vector). ||i|| = 1
   *
   * <p>OUT: The j vector will contain an vector that is colinear to the last/first base pair
   * connection on the side of this endpoint. The vector will be oriented to the side of the given
   * endpoint. ||j|| = 1
   */
   ...
   * http://en.wikipedia.org/wiki/Bezier_curve#Cubic_B.C3.A9zier_curves For each of the curve end
   * points, there is the last/first point of the curve and a point which gives the direction and
   * length of the tangent vector on that side. This two points are respectively curveEndPoint and
   * curveVectorOtherPoint. IN: Argument helixVector is the vector formed by the helix, in the right
   * direction for our sequence. IN: Argument curveEndPoint is the position of the endpoint on the
   * helix. OUT: Argument curveVectorOtherPoint must be allocated and the values will be modified.
   */
  private void computeBezierTangentVectorTarget(
      EdgeEndPoint endPoint, Point2D.Double curveEndPoint, Point2D.Double curveVectorOtherPoint)
      throws RNATemplateDrawingAlgorithmException {
  ...
  private Point2D.Double computeLengthIncreaseDelta(
    ...
    return new Point2D.Double(
        i.x * (realLength - templateLength), i.y * (realLength - templateLength));
  }
   ...
   * @param o This point coordinates will be set the origin of the helix (or not if null), ie. the
   *     point in the middle of the base pair with the two most extreme bases.
   * @param i Will be set to the normalized helix vector. (nothing done if null)
   * @param j Will be set to the normalized helix base pair vector (5' -> 3'). (nothing done if
   *     null)
   */
  private void computeTemplateHelixVectors(
    ...
    if (i != null || j != null) {
      ...
      if (i == null) i = new Point2D.Double();
  ...
  private double estimateBulgeArcLength(int firstBase, int lastBase) {
    if (firstBase + 1 == lastBase) return RNA.LOOP_DISTANCE; // there is actually no bulge
    double len = 0.0;
    int k = firstBase;
    while (k < lastBase) {
  ...
  private double computeHelixTemplateLength(RNATemplateHelix helix) {
    return Math.hypot(
        helix.getStartPosition().x - helix.getEndPosition().x,
  ...
  private double computeHelixRealLength(int[] basesInHelixArray, boolean straightBulges) {
    return drawHelixLikeTemplateHelix(
        basesInHelixArray, null, null, null, null, 0, null, straightBulges);
  }
   ...
   * of an helix) and how many should have a consecutive unpaired bases length? Returns an array
   * with three elements: - answer to first question - answer to second question - sum of both, ie.
   * total number of intervals (this is NOT lastBase-firstBase because bases deep in helices do not
   * count)
   */
  ...
  /**
   * Draw the given helix (given as a *SORTED* array of indexes) like defined in the given template
   * helix. OUT: The bases positions are not changed in fact, instead the coords and centers arrays
   * are modified. IN: The helix origin position is multiplied by scaleHelixOrigin and
   * translateVectors.get(helix) is added. RETURN VALUE: The length of the drawn helix.
   *
   * @param straightBulges
   */
          ...
          if (coords != null) {
            ...
            for (int side = 0; side < 2; side++) {
              ...
              if (side == 0) {
                ...
                pstart.setLocation(
                    o.x + accDelta.x + i.x * delta + j.x * RNA.BASE_PAIR_DISTANCE,
              ...
              if (centerOnBisect > -1000) {
                ...
                double LD =
                    Math.max(
                        (delta - counts.nBP * RNA.BASE_PAIR_DISTANCE) / (double) counts.nLD, 0);
                ...
                // System.out.println("len=" + len + " delta=" + delta + " d(pstart,pend)=" +
                // Math.hypot(pend.x-pstart.x, pend.y-pstart.y));
              }
              ...
              // If so, use drawLoop() to draw it.
              {
                ...
                while (b < lastBase) {
                  ...
                  if (b < l && l < lastBase) {
                    ...
                    double beta =
                        MiscGeom.angleFromVector(b2pos.x - b1pos.x, b2pos.y - b1pos.y)
                            - Math.PI / 2
                            + (flipped ? Math.PI : 0);
                    Point2D.Double loopCenter =
                        new Point2D.Double((b1pos.x + b2pos.x) / 2, (b1pos.y + b2pos.y) / 2);
                    rna.drawLoop(
                        b,
                        l,
                        loopCenter.x,
  ...
  private boolean drawOnStraightLine(
    ...
    UnpairedLineCounts counts = countUnpairedLine(firstBase, lastBase);
    double LD =
        Math.max((vectorNorm - counts.nBP * RNA.BASE_PAIR_DISTANCE) / (double) counts.nLD, 0);
   ...
   * http://en.wikipedia.org/wiki/Bezier_curve#Cubic_B.C3.A9zier_curves Here we give this four
   * points and an array of bases indexes (which must be indexes in this RNA sequence) which will be
   * moved to be on the Bezier curve. The bases positions are not changed in fact, instead the
   * coords and centers arrays are modified. If cancelIfNotGood, draw only if it would look good,
  ...
  private boolean drawOnBezierCurve(
    ...
    double LD =
        Math.max((curveLength - counts.nBP * RNA.BASE_PAIR_DISTANCE) / (double) counts.nLD, 0);
  ...
  private void drawOnEllipse(
    ...
    if (reverse) {
      ...
      tranform1.scale(1, -1);
      tranform1.transform(
          sequenceBasesCoords, 0, sequenceBasesCoords, 0, sequenceBasesCoords.length);
    }
   ...
   *
   * <p>Bezier curve: A Bezier curve can be defined by four points, see
   * http://en.wikipedia.org/wiki/Bezier_curve#Cubic_B.C3.A9zier_curves
   *
   * <p>Straight line: If P1 and P2 are null, the bases are drawn on a straight line.
   *
   * <p>OUT: The bases positions are not changed in fact, instead the coords and centers arrays are
   * modified.
   *
   * <p>Argument reverse says if we should reverse the direction of inserted helices.
   */
    ...
    for (int k = 0; k < n - 1; k++) {
      ...
      if (_listeBases.get(b1).getElementStructure() == b2) {
        ...
        rna.drawLoop(
            ...
            straightBulges);
        if (reverse) {
          Point2D.Double symAxisVect =
              new Point2D.Double(coords[b2].x - coords[b1].x, coords[b2].y - coords[b1].y);
  ...
  /**
   * Compute the symmetric of all the points from first to last in the points array relative to the
   * line that goes through p and has director vector v. The array is modified in-place.
   */
  private static void symmetric(
      Point2D.Double p, Point2D.Double v, Point2D.Double[] points, int first, int last) {
    // ||v||^2
  ...
  private void computeHelixTranslations(
    ...
    if (nvt instanceof RNANodeValueTemplateBasePair) {
      ...
      if (!translateVectors.containsKey(helix)) {
        ...
        Point2D.Double helixDeltaVector =
            computeLengthIncreaseDelta(basesInHelixArray, helix, straightBulges);
        newDeltaVector =
            new Point2D.Double(
                parentDeltaVector.x + helixDeltaVector.x, parentDeltaVector.y + helixDeltaVector.y);
      }
    }
  ...
  private Map<RNATemplateHelix, Point2D.Double> computeHelixTranslations(
      ...
      RNATemplateMapping mapping, // IN
      boolean straightBulges) {
    Map<RNATemplateHelix, Point2D.Double> translateVectors =
        new HashMap<RNATemplateHelix, Point2D.Double>();
    computeHelixTranslations(
        tree, translateVectors, mapping, new Point2D.Double(0, 0), straightBulges);
