...
/**
 * The display Style of a rna base with the base name font, the ouline, innerline, number and name
 * color
 *
 * @author darty
 */
public class ModelBaseStyle implements Cloneable, Serializable {
  ...
  private Color _base_outline_color, _base_inner_color, _base_number_color, _base_name_color;
  ...
  public void toXML(TransformerHandler hd) throws SAXException {
    ...
    atts.addAttribute(
        "", "", XML_VAR_OUTLINE_NAME, "CDATA", "" + XMLUtils.toHTMLNotation(_base_outline_color));
    atts.addAttribute(
        "", "", XML_VAR_INNER_NAME, "CDATA", "" + XMLUtils.toHTMLNotation(_base_inner_color));
    atts.addAttribute(
        "", "", XML_VAR_NUMBER_NAME, "CDATA", "" + XMLUtils.toHTMLNotation(_base_number_color));
    atts.addAttribute(
        "", "", XML_VAR_NAME_NAME, "CDATA", "" + XMLUtils.toHTMLNotation(_base_name_color));
   ...
   *
   * @param outline The out line color of the base
   * @param inner The inner line color of the base
   * @param number The number color of the base
   * @param name The name color of the base
   * @param font The name font of the base
   */
  public ModelBaseStyle(Color outline, Color inner, Color number, Color name, Font font) {
      ...
      if (namesArray.get(i).toLowerCase().equals(PARAM_INNER_COLOR)) {
        try {
          ...
          throw new ExceptionParameterError(
              e.getMessage(), "Bad inner color Syntax:" + valuesArray.get(i));
        }
        ...
        try {
          ...
          throw new ExceptionParameterError(
              e.getMessage(), "Bad name color Syntax:" + valuesArray.get(i));
        }
      } else if (namesArray.get(i).toLowerCase().equals(PARAM_NUMBER_COLOR)) {
        try {
          setBaseNumberColor(getSafeColor(valuesArray.get(i)));
        } catch (NumberFormatException e) {
          throw new ExceptionParameterError(
              e.getMessage(), "Bad numbers color Syntax:" + valuesArray.get(i));
        }
      } else if (namesArray.get(i).toLowerCase().equals(PARAM_OUTLINE_COLOR)) {
        try {
          setBaseOutlineColor(getSafeColor(valuesArray.get(i)));
        } catch (NumberFormatException e) {
          throw new ExceptionParameterError(
              e.getMessage(), "Bad outline color Syntax:" + valuesArray.get(i));
        }
      } else
        throw new ExceptionModeleStyleBaseSyntaxError("Unknown parameter:" + namesArray.get(i));
    }
  }
  ...
  public static Integer StyleToInteger(String s) {
    Integer style;
    if (s.toLowerCase().equals("italic")) style = Font.ITALIC;
    else if (s.toLowerCase().equals("bold")) style = Font.BOLD;
    else if (s.toLowerCase().equals("plain")) style = Font.PLAIN;
    else style = null;
    return style;
  }
}
