...
public class ModeleBP implements Serializable, Comparable<ModeleBP> {
  ...
  public boolean isCanonical() {
    return (_edge5 == Edge.WC) && (_edge3 == Edge.WC) && (_stericity == Stericity.CIS);
  }
  ...
  public ModeleBase getPartner(ModeleBase mb) {
    if (mb == _partner3) return _partner5;
    else return _partner3;
  }
  ...
  public void assignParameters(String parametersValue)
    ...
    for (int i = 0; i < namesArray.size(); i++) {
      if (namesArray.get(i).toLowerCase().equals(PARAM_COLOR)) {
        try {
          _style.setCustomColor(ModelBaseStyle.getSafeColor(valuesArray.get(i)));
        ...
        try {
          ...
          throw new ExceptionParameterError(
              e.getMessage(), "Bad value for bp thickness:" + valuesArray.get(i));
        }
        ...
        if (s.toLowerCase().equals(VALUE_WATSON_CRICK)) {
          ...
          setEdge5(Edge.SUGAR);
        } else throw new ExceptionParameterError("Bad value for edge:" + valuesArray.get(i));
        ...
        } else throw new ExceptionParameterError("Bad value for stericity:" + valuesArray.get(i));
      } else
