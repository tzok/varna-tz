...
public class ModeleBPStyle implements Serializable {
  ...
  /**
   * Returns the current custom color if such a color is defined to be used (through
   * setCustomColor), or returns the default color.
   *
   * @param def - The default color is no custom color is defined
  ...
  public double getThickness(double def) {
    if (_thickness > 0) return _thickness;
    else return def;
  }
