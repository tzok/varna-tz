...
public class ModeleBaseNucleotide extends ModeleBase {
   ...
   * Creates a new rna base with the nitrogenous base
   *
   * @param s The full label, potentially requiring further decoding
   * @param index The index of this base
   * @param baseNumber The number of this base, which may differ from the index (e.g. discontinuous
   *     numbering)
   */
  ...
  public ModeleBaseNucleotide(Point2D.Double coord, int index) {
    this(
        new Point2D.Double(coord.getX(), coord.getY()),
        new Point2D.Double(),
        true,
        "",
        new ModelBaseStyle(),
   ...
   *
   * @param mb The base to copy
   */
  public ModeleBaseNucleotide(ModeleBaseNucleotide mb, int index) {
    this(
        new Point2D.Double(mb.getCoords().getX(), mb.getCoords().getY()),
        new Point2D.Double(mb.getCenter().getX(), mb.getCenter().getY()),
        true,
        mb.getBase(),
        mb.getStyleBase(),
        mb.getElementStructure(),
        index);
  }

  public ModeleBaseNucleotide(
              ...
              default:
                if ((state == STATE_SPECIAL_CHARS_STATES.SUBSCRIPT) && _subscripts.containsKey(c))
                  result += _subscripts.get(c);
                    ...
                    && _superscripts.containsKey(c)) result += _superscripts.get(c);
                else result += c;
                state = STATE_SPECIAL_CHARS_STATES.NORMAL;
                break;
            }
