...
public class VARNASecDraw {
  ...
  public class PairedPortion extends Portion {
    ...
    public GeneralPath getOutline(RNA r, boolean local) {
      ...
      gp.moveTo((float) p1.x, (float) p1.y);
      gp.lineTo((float) p2.x, (float) p2.y);
      if (!local) gp.append(_r.getOutline(r), true);
      gp.lineTo((float) p3.x, (float) p3.y);
      gp.lineTo((float) p4.x, (float) p4.y);
  ...
  */
  ...
  public int[] nextPlacement(int[] p) throws Exception {
    ...
    if (i < 0) throw new Exception("No more placement available");
    p[i]++;
    i++;
    while (i < p.length) {
      p[i] = p[i - 1] + 1;
        ...
        if (p instanceof PairedPortion) {
          ...
          for (int j = 0; j < pp._len; j++) {
            ...
            r.get_listeBases()
                ...
                .setCoords(
                    new Point2D.Double(
                        nx + r.BASE_PAIR_DISTANCE * vy / 2.0,
                        ny - r.BASE_PAIR_DISTANCE * vx / 2.0));
            r.get_listeBases()
                .get(i2)
                .setCoords(
                    new Point2D.Double(
    ...
    public void reflectCoordinates() {
      ...
      if (_p._len > 0) {
        ...
        for (int n = 0; n < _p._len; n++) {
          ...
          mbLeft.setCoords(
              new Point2D.Double(startLeft.x + n * vx * interval, startLeft.y + n * vy * interval));
          ModeleBase mbRight = mbl.get(j);
          mbRight.setCoords(
              new Point2D.Double(
                  startRight.x + n * vx * interval, startRight.y + n * vy * interval));
        }
      }
      for (int i = 0; i < _children.size(); i++) {
      ...
      if (p instanceof PairedPortion) {
        ...
        for (int j = 0; j < pp._len; j++) {
          ...
          r.get_listeBases()
              ...
              .setCoords(
                  new Point2D.Double(
                      nx + r.BASE_PAIR_DISTANCE * vy / 2.0, ny - r.BASE_PAIR_DISTANCE * vx / 2.0));
          r.get_listeBases()
              .get(i2)
              .setCoords(
                  new Point2D.Double(
  ...
  public RNATree drawRNA(double dirAngle, RNA r) {
    ...
    try {
      root =
          new HelixEmbedding(new Point2D.Double(0.0, 0.0), new PairedPortion(0, 0, 0, t), r, null);
