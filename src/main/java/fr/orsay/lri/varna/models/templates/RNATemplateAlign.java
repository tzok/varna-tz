...
/**
 * This class is about the alignment between a tree of RNANodeValue2 and a tree of
...
public class RNATemplateAlign {
  ...
  private static boolean canBePartOfAnHelix(RNANodeValue2 leftNodeValue) {
    return (leftNodeValue != null)
        && leftNodeValue.isSingleNode()
        && leftNodeValue.getNode().getRightBasePosition() > 0;
  }
  ...
  /**
   * This method takes an alignment between a tree of RNANodeValue2 of RNANodeValue and a tree of
   * RNANodeValue2 of RNANodeValueTemplate, and the original RNA object that was used to create the
   * first tree in the alignment. It returns the corresponding RNATemplateMapping.
   */
  public static RNATemplateMapping makeTemplateMapping(
      TreeAlignResult<RNANodeValue2, RNANodeValueTemplate> alignResult, RNA rna)
    ...
    // Map sequences and helices together, without managing pseudoknots
    {
      ...
      LinkedList<Tree<AlignedNode<RNANodeValue2, RNANodeValueTemplate>>> remainingNodes =
          new LinkedList<Tree<AlignedNode<RNANodeValue2, RNANodeValueTemplate>>>();
      ...
      while (!remainingNodes.isEmpty()) {
          ...
          if (rightNode != null && rightNode.getValue() != null) {
            ...
            if (rightNodeValue instanceof RNANodeValueTemplateBasePair
              ...
              RNATemplateUnpairedSequence sequence =
                  ((RNANodeValueTemplateSequence) rightNodeValue).getSequence();
        ...
        if (n > 0) {
          ...
          ArrayList<Tree<AlignedNode<RNANodeValue2, RNANodeValueTemplate>>> addToStack1 =
              new ArrayList<Tree<AlignedNode<RNANodeValue2, RNANodeValueTemplate>>>();
          ArrayList<Tree<AlignedNode<RNANodeValue2, RNANodeValueTemplate>>> addToStack2 =
              new ArrayList<Tree<AlignedNode<RNANodeValue2, RNANodeValueTemplate>>>();
          for (int i = 0; i < n; i++) {
            ...
            if (RNAchild != null
                ...
                && (canBePartOfAnHelix(RNAchild.getValue())
                    || canBePartOfABrokenHelix(RNAchild.getValue()))) {
              helixChildren++;
              ...
              if (rightNode != null && rightNode.getValue() != null) {
                ...
                if (rightNodeValue instanceof RNANodeValueTemplateBrokenBasePair
                  ...
                  RNATemplateHelix helix =
                      ((RNANodeValueTemplateBrokenBasePair) rightNodeValue).getHelix();
                  currentHelix = helix;
                ...
                if (canBePartOfABrokenHelix(leftNodeValue)) {
                  if (currentHelix != null) {
                    ...
                    // contains a longer helix than in the template
                    tempPKMapping.addCouple(
                        leftNodeValue.getNode().getLeftBasePosition(), currentHelix);
                  } else {
                    // Maybe this left node is part of an helix
                    // which is smaller in the template
                    nodesInSameHelix.add(leftNodeValue);
  ...
  public static void printMapping(
      RNATemplateMapping mapping, RNATemplate template, String sequence) {
  ...
  public static TreeAlignResult<RNANodeValue2, RNANodeValueTemplate> alignRNAWithTemplate(
    ...
    try {
      ...
      TreeAlign<RNANodeValue2, RNANodeValueTemplate> treeAlign =
          new TreeAlign<RNANodeValue2, RNANodeValueTemplate>(new RNANodeValue2TemplateDistance());
      TreeAlignResult<RNANodeValue2, RNANodeValueTemplate> result =
          treeAlign.align(rnaAsTree, templateAsTree);
      return result;
  ...
  public static RNATemplateMapping mapRNAWithTemplate(RNA rna, RNATemplate template)
      throws RNATemplateDrawingAlgorithmException {
    try {
      TreeAlignResult<RNANodeValue2, RNANodeValueTemplate> alignResult =
          RNATemplateAlign.alignRNAWithTemplate(rna, template);
      RNATemplateMapping mapping = RNATemplateAlign.makeTemplateMapping(alignResult, rna);
      return mapping;
    } catch (RNATemplateMappingException e) {
      e.printStackTrace();
      throw (new RNATemplateDrawingAlgorithmException(
          "RNATemplateMappingException: " + e.getMessage()));
    }
  }
  ...
  /**
   * Convert a list of integers into an array of integers. The returned arrays is freshly allocated.
