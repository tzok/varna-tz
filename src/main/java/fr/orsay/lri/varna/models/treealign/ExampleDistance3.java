...
public class ExampleDistance3 implements TreeAlignLabelDistanceSymmetric<RNANodeValue2> {
  public double f(RNANodeValue2 v1, RNANodeValue2 v2) {
    if (v1 == null) {
      ...
      if (v2 == null) {
        ...
        return (!l1.equals(l2) ? 0.5 : 0) + (!r1.equals(r2) ? 0.5 : 0);
      }
    }
  }
  ...
  public class SequenceAlignResult {
    ...
    /**
     * The result array is a matrix of height 2 and width at most length(sequence A) +
     * length(sequence B). with result[0] is the alignment for A and result[1] the alignment for B.
     * The alignment consists int the indexes of the original bases positions, with -1 when there is
     * no match.
     */
  ...
  /**
   * Align two sequences contained in nodes. Both nodes have to be non-single nodes, otherwise an
  ...
  /**
   * Align sequences using the Needleman-Wunsch algorithm. Time: O(A.length * B.length) Space:
   * O(A.length * B.length) Space used by the returned object: O(A.length + B.length)
   */
  public SequenceAlignResult alignSequences(char[] A, char[] B) {
    ...
    for (int i = 0; i <= la; i++) F[i][0] = d * i;
    for (int j = 0; j <= lb; j++) F[0][j] = d * j;
    for (int i = 1; i <= la; i++)
      for (int j = 1; j <= lb; j++) {
        double min;
        int decision;
