...
/**
 * We use the following convention: If the node is marked by a couple (n,m) these integers are
 * stored in leftBasePosition and rightBasePosition (ie. we have a pair of bases), if only one value
 * is present, it is stored in leftBasePosition and rightBasePosition contains -1 (ie. we have a
 * non-paired base). The right and left nucleotides follow the same rule, but are optional, and '_'
 * is used as undefined instead of -1. Note that it is part of the contract of this class that
 * default values are -1 and _.
 *
