...
public class RNATree {
  /**
   * Convert an RNA object into a RNA tree. The root node will have a null value because it is a
   * fake node added to have a tree (otherwise we would have a forest).
   */
        ...
        if (indexOfAssociatedBase >= 0) {
          if (indexOfAssociatedBase > i) {
            ...
            if (base instanceof ModeleBaseNucleotide) {
              ...
              childValue.setRightNucleotide(
                  ((ModeleBaseNucleotide) rna.getBaseAt(indexOfAssociatedBase)).getBase());
            }
            i++;
          ...
          for (ModeleBP auxBasePair : auxBasePairs) {
            if (auxBasePair.isCanonical()) {
              ...
              if (i == partner5) {
                ...
                System.err.println(
                    "Warning: Base index is "
                        + i
                        + " but neither endpoint matches it (edge endpoints are "
                        + partner5
                        + " and "
                        + partner3
                        + ").");
  ...
  public static Mapping mappingFromAlignment(
      Tree<AlignedNode<RNANodeValue, RNANodeValue>> alignment) throws MappingException {
  ...
  private static class ConvertToMapping {
    ...
    public Mapping convert(Tree<AlignedNode<RNANodeValue, RNANodeValue>> tree)
    ...
    private void convertSubTree(Tree<AlignedNode<RNANodeValue, RNANodeValue>> tree)
        throws MappingException {
