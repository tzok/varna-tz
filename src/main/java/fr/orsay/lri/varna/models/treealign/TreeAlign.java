...
/**
 * Tree alignment algorithm. This class implements the tree alignment algorithm for ordered trees
 * explained in article: T. Jiang, L. Wang, K. Zhang, Alignment of trees - an alternative to tree
 * edit, Theoret. Comput. Sci. 143 (1995). Other references: - Claire Herrbach, Alain Denise and
 * Serge Dulucq. Average complexity of the Jiang-Wang-Zhang pairwise tree alignment algorithm and of
 * a RNA secondary structure alignment algorithm. Theoretical Computer Science 411 (2010) 2423-2432.
 *
 * <p>Our implementation supposes that the trees will never have more than 32000 nodes and that the
 * total distance will never require more significant digits that a float (single precision) has.
 *
...
public class TreeAlign<ValueType1, ValueType2> {
  ...
  private class TreeData<ValueType> {
    ...
    /**
     * The number of children of a node is called the node degree. This variable is the maximum node
     * degree in the tree.
     */
    public int degree = -1;

    /**
     * The number of children of a node is called the node degree. degree[i] is the degree of node
     * i, with i being an index in nodes.
     */
    public int[] degrees;
  ...
  /**
   * Create a TreeAlignSymmetric object, which can align trees. The distance function will be called
   * only once on every pair of nodes. The result is then kept in a matrix, so you need not manage
   * yourself a cache of f(value1, value2). Note that it is permitted to have null values on nodes,
   * so comparing a node with a non-null value with a node with a null value will give the same cost
   * as to insert the first node. This can be useful if you tree has "fake" nodes.
   *
  ...
  private class ConvertTreeToArray<ValueType> {
    ...
    private void convertTreeToArrayAux(
        Tree<ValueType> subtree, int[] siblingIndexes, int siblingNumber)
    ...
    /**
     * Reads: treeData.tree Computes: treeData.nodes, treeData.degree, treeData.degrees
     * treeData.fathers, treeData.children, treeData.size, treeData.values Converts a tree to an
     * array of nodes, in postorder. We also compute the maximum node degree in the tree.
     *
     * @throws TreeAlignException
     */
  ...
  private class Aligner {
    ...
    /**
     * DF1[i][j_t] is DFL for (i,j,s,t) with s=0. See description of DFL in
     * Aligner.computeAlignmentP1(). DF1 and DF2 are the "big" arrays, ie. those that may the space
     * complexity what it is.
     */
    private float[][][][] DF1;

    /**
     * DF2[j][i_s] is DFL for (i,j,s,t) with t=0. See description of DFL in
     * Aligner.computeAlignmentP1().
     */
    private float[][][][] DF2;

    /**
     * This arrays have the same shape as respectively DF1. They are used to remember which term in
     * the minimum won, so that we can compute the alignment. Decision1 is a case number (< 10) and
     * Decision2 is a child index, hence the types.
     */
    ...
    /**
     * This arrays have the same shape as respectively DF2. They are used to remember which term in
     * the minimum won, so that we can compute the alignment.
     */
    ...
    /**
     * Distances between subtrees. DT[i][j] is the distance between the subtree rooted at i in the
     * first tree and the subtree rooted at j in the second tree.
     */
    private float[][] DT;

    /**
     * This array has the same shape as DT, but is used to remember which case gave the minimum, so
     * that we can later compute the alignment.
     */
    ...
    /**
     * Distances between labels. DL[i][j] is the distance labelDist.f(value(T1[i]), value(T2[i])).
     * By convention, we say that value(T1[|T1|]) = null and value(T2[|T2|]) = null
     */
    private float[][] DL;

    /**
     * DET1[i] is the distance between the empty tree and T1[i] (the subtree rooted at node i in the
     * first tree).
     */
    private float[] DET1;
    ...
    /**
     * DEF1[i] is the distance between the empty forest and F1[i] (the forest of children of node i
     * in the first tree).
     */
    private float[] DEF1;
    ...
    private void computeAlignmentP1(int i, int s, int m_i, int j, int t, int n_j, int DFx) {
      /**
       * DFL[pr][qr] is D(F1[i_s, i_p], F2[j_t, j_q]) where p=s+pr-1 and q=t+qr-1 (ie. pr=p-s+1 and
       * qr=q-t+1) By convention, F1[i_s, i_{s-1}] and F2[j_t, j_{t-1}] are the empty forests. Said
       * differently, DFL[pr][qr] is the distance between the forest of the pr first children of i,
       * starting with child s (first child is s = 0), and the forest of the qr first children of j,
       * starting with child t (first child is t = 0). This array is allocated for a fixed value of
       * (i,j,s,t).
       */
      float[][] DFL;

      /**
       * Same shape as DFL, but to remember which term gave the min, so that we can later compute
       * the alignment.
       */
      byte[][] DFLDecisions1;
      short[][] DFLDecisions2;
      ...
      for (int p = s; p < m_i; p++) {
        ...
        for (int q = t; q < n_j; q++) {
          ...
          {
            float minCandidate = DFL[p - s][q - t] + DT[i_p][j_q];
            if (minCandidate < min) {
              min = minCandidate;
              decision1 = 3;
            }
    ...
    public float align() throws TreeAlignException {
      ...
      for (int i = 0; i < treeData1.size; i++) {
        ...
        for (int j = 0; j < treeData2.size; j++) {
          ...
          DL[i][j] =
              (float)
                  labelDist.f((ValueType1) treeData1.values[i], (ValueType2) treeData2.values[j]);
    ...
    /**
     * Align F1[i_s,i_p] with F2[j_t,j_q]. If p = s-1, by convention it means F1[i_s,i_p] = empty
     * forest. Idem for q=t-1.
     */
    private List<Tree<AlignedNode<ValueType1, ValueType2>>> computeForestAlignment(
      ...
      if (p == s - 1) { // left forest is the empty forest
        List<Tree<AlignedNode<ValueType1, ValueType2>>> result =
            new ArrayList<Tree<AlignedNode<ValueType1, ValueType2>>>();
        for (int k = t; k <= q; k++) {
        ...
        if (q == t - 1) { // right forest is the empty forest
          ...
          int decision1, k;
          if (s == 0) {
            decision1 = DF1Decisions1[i][treeData2.children[j][t]][p - s + 1][q - t + 1];
            k = DF1Decisions2[i][treeData2.children[j][t]][p - s + 1][q - t + 1];
          } else if (t == 0) {
            decision1 = DF2Decisions1[j][treeData1.children[i][s]][p - s + 1][q - t + 1];
            k = DF2Decisions2[j][treeData1.children[i][s]][p - s + 1][q - t + 1];
          } else {
            ...
            case 3:
              {
                ...
                result.add(
                    computeTreeAlignment(treeData1.children[i][p], treeData2.children[j][q]));
                return result;
              }
            case 4:
              {
                ...
                int j_q = treeData2.children[j][q];
                Tree<AlignedNode<ValueType1, ValueType2>> insertedNode =
                    new Tree<AlignedNode<ValueType1, ValueType2>>();
                AlignedNode<ValueType1, ValueType2> insertedNodeValue =
                    new AlignedNode<ValueType1, ValueType2>();
                ...
                insertedNode.replaceChildrenListBy(
                    computeForestAlignment(i, k, p, j_q, 0, treeData2.degrees[j_q] - 1));
            ...
            case 5:
              {
                ...
                int i_p = treeData1.children[i][p];
                Tree<AlignedNode<ValueType1, ValueType2>> deletedNode =
                    new Tree<AlignedNode<ValueType1, ValueType2>>();
                AlignedNode<ValueType1, ValueType2> deletedNodeValue =
                    new AlignedNode<ValueType1, ValueType2>();
                ...
                deletedNode.replaceChildrenListBy(
                    computeForestAlignment(i_p, 0, treeData1.degrees[i_p] - 1, j, k, q));
        ...
        case 1:
          {
            ...
            AlignedNode<ValueType1, ValueType2> alignedNode =
                new AlignedNode<ValueType1, ValueType2>();
            alignedNode.setLeftNode(null);
            alignedNode.setRightNode(treeData2.nodes[j]);
            root.setValue(alignedNode);
        ...
        case 3:
          {
            ...
            List<Tree<AlignedNode<ValueType1, ValueType2>>> children =
                computeForestAlignment(
                    i, 0, treeData1.degrees[i] - 1, j, 0, treeData2.degrees[j] - 1);
  ...
  /**
   * Align T1 with T2, computing both the distance and the alignment. Time:
   * O(|T1|*|T2|*(deg(T1)+deg(T2))^2) Space: O(|T1|*|T2|*(deg(T1)+deg(T2))) Average (over possible
   * trees) time: O(|T1|*|T2|)
   *
   ...
   * @throws TreeAlignException
   */
  public TreeAlignResult<ValueType1, ValueType2> align(Tree<ValueType1> T1, Tree<ValueType2> T2)
      throws TreeAlignException {
  ...
  /**
   * Takes a alignment, and compute the distance between the two original trees. If you have called
   * align(), the result object already contains the distance D and the alignment A. If you call
