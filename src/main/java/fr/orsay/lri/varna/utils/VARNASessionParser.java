...
public class VARNASessionParser extends DefaultHandler {
  ...
  public void startElement(String uri, String localName, String qName, Attributes attributes)
    ...
    if (qName.equals(VARNAPanel.XML_ELEMENT_NAME)) {
      ...
      if (backbone != null) {
        ...
        ModeleBackboneElement.BackboneType type =
            ModeleBackboneElement.BackboneType.getType(
      ...
      if (rna != null) {
        ...
        mbn.setColorie(
            Boolean.parseBoolean(attributes.getValue(ModeleBase.XML_VAR_CUSTOM_DRAWN_NAME)));
      ...
      if (rna != null) {
        ...
        mbc.set_appartenance(
            Integer.parseInt(attributes.getValue(ModeleBasesComparison.XML_VAR_MEMBERSHIP_NAME)));
        mbc.setColorie(
            Boolean.parseBoolean(attributes.getValue(ModeleBase.XML_VAR_CUSTOM_DRAWN_NAME)));
      ...
      if (inSecStr) rna.addBP(i5, i3, mbp);
      else rna.addBPAux(i5, i3, mbp);
      ...
      cpa.setIntensity(
          Double.parseDouble(attributes.getValue(ChemProbAnnotation.XML_VAR_INTENSITY_NAME)));
      cpa.setType(
          ChemProbAnnotationType.valueOf(
              attributes.getValue(ChemProbAnnotation.XML_VAR_TYPE_NAME)));
      cpa.setOut(
          Boolean.parseBoolean(attributes.getValue(ChemProbAnnotation.XML_VAR_OUTWARD_NAME)));
      ...
      ta.setAngleInDegres(
          Double.parseDouble(attributes.getValue(TextAnnotation.XML_VAR_ANGLE_NAME)));
      ta.setType(t);
      ...
      hra.setOutlineColor(
          Color.decode(attributes.getValue(HighlightRegionAnnotation.XML_VAR_OUTLINE_NAME)));
      hra.setFillColor(
          Color.decode(attributes.getValue(HighlightRegionAnnotation.XML_VAR_FILL_NAME)));
      hra.setRadius(
          Double.parseDouble(attributes.getValue(HighlightRegionAnnotation.XML_VAR_RADIUS_NAME)));
      ...
      if (contextContains(TextAnnotation.XML_ELEMENT_NAME)) {
        if (ta != null) ta.setAncrage(vp.x, vp.y);
      ...
      msb.setBaseOutlineColor(
          Color.decode(attributes.getValue(ModelBaseStyle.XML_VAR_OUTLINE_NAME)));
      ...
      boolean customColor =
          Boolean.parseBoolean(attributes.getValue(ModeleBPStyle.XML_VAR_CUSTOM_STYLED_NAME));
      if (customColor)
    ...
    if (qName.equals(ModeleBaseNucleotide.XML_VAR_CONTENT_NAME)) {
      if (_buffer == null) {
        throw new SAXException(
            "Invalid location for tag " + ModeleBaseNucleotide.XML_VAR_CONTENT_NAME);
      }
      if (mbn == null) {
        throw new SAXException(
            "Invalid location for tag " + ModeleBaseNucleotide.XML_VAR_CONTENT_NAME);
  ...
  public void characters(char[] ch, int start, int length) throws SAXException {
    ...
    if (_buffer != null) _buffer.append(lecture);
  }
