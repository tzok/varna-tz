...
public class VueAnnotation {
   ...
   *
   * @param vp
   * @param limited if true, lets custom position and angle.
   */
   ...
   * @param vp
   * @param textAnnot
   * @param reduite if true, lets custom position and angle.
   * @param newAnnotation if true, deleted if cancelled.
   */
  public VueAnnotation(
      VARNAPanel vp, TextAnnotation textAnnot, boolean reduite, boolean newAnnotation) {
    ...
    ySlider =
        new JSlider(
            JSlider.HORIZONTAL,
            0,
            (int) (_vp.getExtendedRNABBox().height),
            Math.max(
                0,
                Math.min(
                    (int) (_vp.getExtendedRNABBox().height),
                    (int) (position.y - _vp.getExtendedRNABBox().y))));
    // Turn on labels at major tick marks.
    ySlider.setMajorTickSpacing(500);
    ySlider.setMinorTickSpacing(100);
    ySlider.setPaintTicks(true);
    ySlider.setPaintLabels(true);
    ySlider.setPreferredSize(new Dimension(400, ySlider.getPreferredSize().height));

    JLabel yValueLabel = new JLabel(String.valueOf((int) position.y - _vp.getExtendedRNABBox().y));
    yValueLabel.setPreferredSize(new Dimension(50, yValueLabel.getPreferredSize().height));
    ySlider.addChangeListener(new ControleurSliderLabel(yValueLabel, false));
    ...
    xSlider =
        new JSlider(
            JSlider.HORIZONTAL,
            0,
            (int) (_vp.getExtendedRNABBox().width),
            Math.max(
                0,
                Math.min(
                    (int) _vp.getExtendedRNABBox().width,
                    (int) (position.x - _vp.getExtendedRNABBox().x))));
    // Turn on labels at major tick marks.
    xSlider.setMajorTickSpacing(500);
    xSlider.setMinorTickSpacing(100);
    xSlider.setPaintTicks(true);
    xSlider.setPaintLabels(true);
    xSlider.setPreferredSize(new Dimension(400, xSlider.getPreferredSize().height));

    JLabel xValueLabel = new JLabel(String.valueOf((int) position.x - _vp.getExtendedRNABBox().x));
    xValueLabel.setPreferredSize(new Dimension(50, xValueLabel.getPreferredSize().height));
    xSlider.addChangeListener(new ControleurSliderLabel(xValueLabel, false));
    ...
    Border border =
        new BasicBorders.FieldBorder(Color.black, Color.black, Color.black, Color.black);
    ...
    rotationSlider =
        new JSlider(JSlider.HORIZONTAL, -360, 360, (int) textAnnotation.getAngleInDegres());
    ...
    rotationSlider.addChangeListener(new ControleurSliderLabel(rotationLabel, false));
  ...
  public void update() {
    ...
    if (textAnnotation.getType() == TextAnnotation.AnchorType.POSITION)
      textAnnotation.setAncrage(
          (double) xSlider.getValue() + _vp.getExtendedRNABBox().x,
          ySlider.getValue() + _vp.getExtendedRNABBox().y);
   ...
   *
   * <p>if validate, just update the annoted text
   *
   * <p>if cancelled : remove the annoted text if it was a new one, otherwise cancel modifications
   *
   * <p>
   */
  public void show() {
