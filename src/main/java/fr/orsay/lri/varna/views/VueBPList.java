...
public class VueBPList extends JPanel implements TableModelListener, ActionListener {
  ...
  private void init() {
    Object[] col = {
      "Sec.Str.", "5' partner", "3' partner", "5' edge", "3' edge", "Orientation", "Remove"
    };
    ...
    table.setDefaultEditor(
        ModeleBP.Edge.class, new DefaultCellEditor(new JComboBox(ModeleBP.Edge.values())));
    table.setDefaultEditor(
        ModeleBP.Stericity.class,
        new DefaultCellEditor(new JComboBox(ModeleBP.Stericity.values())));
  ...
  private class BPTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int col) {
      ...
      if (col == 0) {
        ...
        return new String(
            "" + mb.getPartner5().getBaseNumber() + "-" + mb.getPartner5().getContent());
      } else if (col == 2) {
        return new String(
            "" + mb.getPartner3().getBaseNumber() + "-" + mb.getPartner3().getContent());
      } else if (col == 3) {
