...
public class VueUI {
  ...
  public void UIToggleFlatExteriorLoop() {
    if (_vp.isModifiable() && _vp.getRNA().get_drawMode() == RNA.DRAW_MODE_RADIATE) {
      Hashtable<Integer, Point2D.Double> bck = backupAllCoords();
      _undoableEditSupport.postEdit(
          new VARNAEdits.RedrawEdit(RNA.DRAW_MODE_RADIATE, _vp, !_vp.getFlatExteriorLoop()));
  ...
  public void UIChooseRNAs(ArrayList<RNA> rnas) {
    if (rnas.size() > 5) {
      ...
      if (JOptionPane.showConfirmDialog(
              _vp, vl, "Select imported sequence/structures", JOptionPane.OK_CANCEL_OPTION)
  ...
  public void UISetColorMapStyle() {
    ...
    if (JOptionPane.showConfirmDialog(
            _vp, cms, "Choose color map style", JOptionPane.OK_CANCEL_OPTION)
        == JOptionPane.OK_OPTION) {
  ...
  public void UILoadColorMapValues() {
    ...
    if (JOptionPane.showConfirmDialog(_vp, cms, "Load base values", JOptionPane.OK_CANCEL_OPTION)
        == JOptionPane.OK_OPTION) {
      try {
  ...
  public void UISetColorMapValues() {
    ...
    if (JOptionPane.showConfirmDialog(_vp, cms, "Choose base values", JOptionPane.OK_CANCEL_OPTION)
        == JOptionPane.OK_OPTION) {
    } else {
      cms.cancelChanges();
    }
  ...
  public void UIManualInput() throws ParseException, ExceptionNonEqualLength {
    if (_vp.isModifiable()) {
      ...
      if (JOptionPane.showConfirmDialog(
              _vp, manualInput.getPanel(), "Input sequence/structure", JOptionPane.OK_CANCEL_OPTION)
          == JOptionPane.OK_OPTION) {
  ...
  public void UISetTitle() {
    if (_vp.isModifiable()) {
      String res = JOptionPane.showInputDialog(_vp, "Input title", _vp.getTitle());
      if (res != null) {
        _vp.setTitle(res);
        _vp.repaint();
      }
  ...
  public void UISetColorMapCaption() {
    if (_vp.isModifiable()) {
      String res =
          JOptionPane.showInputDialog(_vp, "Input new color map caption", _vp.getColorMapCaption());
      if (res != null) {
      ...
      if (_vp.isComparisonMode()) {
        String res =
            JOptionPane.showInputDialog(
                _vp,
                "Input base",
                ((ModeleBasesComparison) _vp.getRNA().get_listeBases().get(i)).getBases());
        if (res != null) {
        ...
        String res =
            JOptionPane.showInputDialog(
                _vp,
                "Input base",
                ((ModeleBaseNucleotide) _vp.getRNA().get_listeBases().get(i)).getBase());
        if (res != null) {
  ...
  FileNameExtensionFilter _varnaFilter =
      new FileNameExtensionFilter("VARNA Session File", "varna", "VARNA");
  FileNameExtensionFilter _bpseqFilter =
      new FileNameExtensionFilter("BPSeq (CRW) File", "bpseq", "BPSEQ");
  FileNameExtensionFilter _ctFilter =
      new FileNameExtensionFilter("Connect (MFold) File", "ct", "CT");
  FileNameExtensionFilter _dbnFilter =
      new FileNameExtensionFilter("Dot-bracket notation (Vienna) File", "dbn", "DBN", "faa", "FAA");

  FileNameExtensionFilter _jpgFilter =
      new FileNameExtensionFilter("JPEG Picture", "jpeg", "jpg", "JPG", "JPEG");
  ...
  FileNameExtensionFilter _xfigFilter =
      new FileNameExtensionFilter("XFig Diagram", "fig", "xfig", "FIG", "XFIG");
  FileNameExtensionFilter _tikzFilter =
  ...
  public void UIExportTIKZ() throws ExceptionExportFailed, ExceptionWritingForbidden {
  ...
  public void UISaveAsDBN() throws ExceptionExportFailed, ExceptionPermissionDenied {
    ...
    if (name != null) _vp.getRNA().saveAsDBN(name, _vp.getTitle());
  }
  ...
  public void UISaveAsCT() throws ExceptionExportFailed, ExceptionPermissionDenied {
    ...
    if (name != null) _vp.getRNA().saveAsCT(name, _vp.getTitle());
  }
  ...
  public void UISaveAsBPSEQ() throws ExceptionExportFailed, ExceptionPermissionDenied {
    ...
    if (name != null) _vp.getRNA().saveAsBPSEQ(name, _vp.getTitle());
  }
   ...
   *
   * @param filtre Allowed extensions
  ...
  public String UIChooseOutputFile(ArrayList<FileNameExtensionFilter> filtre) {
    ...
    if (fc.showSaveDialog(_vp) == JFileChooser.APPROVE_OPTION) {
      ...
      String extension = _vp.getPopupMenu().get_controleurMenu().getExtension(fc.getSelectedFile());
  ...
  public void UISetBorder() {
    ...
    _vp.repaint();
    if (JOptionPane.showConfirmDialog(
            _vp, border.getPanel(), "Set new border size", JOptionPane.OK_CANCEL_OPTION)
        != JOptionPane.OK_OPTION) {
  ...
  public void UISetBackground() {
    Color c = JColorChooser.showDialog(_vp, "Choose new background color", _vp.getBackground());
    if (c != null) {
      _vp.setBackground(c);
      _vp.repaint();
    }
  ...
  public void UIZoomIn() {
    ...
    double newZoom = Math.min(VARNAConfig.MAX_ZOOM, _actualZoom * _actualAmount);
    double ratio = newZoom / _actualZoom;
    Point newTrans =
        new Point((int) (_actualTranslation.x * ratio), (int) (_actualTranslation.y * ratio));
    _vp.setZoom(newZoom);
  ...
  public void UIZoomOut() {
    ...
    double _actualAmount = _vp.getZoomIncrement();
    Point _actualTranslation = _vp.getTranslation();
    double newZoom = Math.max(_actualZoom / _actualAmount, VARNAConfig.MIN_ZOOM);
    double ratio = newZoom / _actualZoom;
    Point newTrans =
        new Point((int) (_actualTranslation.x * ratio), (int) (_actualTranslation.y * ratio));
    _vp.setZoom(newZoom);
  ...
  public void UICustomZoom() {
    ...
    _vp.repaint();
    if (JOptionPane.showConfirmDialog(
            _vp, zoom.getPanel(), "Set zoom", JOptionPane.OK_CANCEL_OPTION)
        != JOptionPane.OK_OPTION) {
      _vp.setZoom(oldZoom);
  ...
  public void UIGlobalRotation() {
    if (_vp.isModifiable()) {
      if (_vp.getRNA().get_listeBases().size() > 0) {
        ...
        if (JOptionPane.showConfirmDialog(
                _vp, rotation.getPanel(), "Rotates the whole RNA", JOptionPane.OK_CANCEL_OPTION)
            != JOptionPane.OK_OPTION) {
  ...
  public void UISetBPStyle() {
    if (_vp.getRNA().get_listeBases().size() > 0) {
      ...
      if (JOptionPane.showConfirmDialog(
              _vp, bpstyle.getPanel(), "Set main base pair style", JOptionPane.OK_CANCEL_OPTION)
          != JOptionPane.OK_OPTION) {
        _vp.setBPStyle(bck);
        _vp.repaint();
      }
  ...
  public void UISetTitleColor() {
    if (_vp.isModifiable()) {
      ...
      Color c = JColorChooser.showDialog(_vp, "Choose new title color", _vp.getTitleColor());
      if (c != null) {
        _vp.setTitleColor(c);
        _vp.repaint();
      }
  ...
  public void UISetBackboneColor() {
    if (_vp.isModifiable()) {
      Color c = JColorChooser.showDialog(_vp, "Choose new backbone color", _vp.getBackboneColor());
      if (c != null) {
  ...
  public void UISetTitleFont() {
    if (_vp.isModifiable()) {
      ...
      if (JOptionPane.showConfirmDialog(
              _vp, font.getPanel(), "New Title font", JOptionPane.OK_CANCEL_OPTION)
          == JOptionPane.OK_OPTION) {
  ...
  public void UISetSpaceBetweenBases() {
    if (_vp.isModifiable()) {
      ...
      if (JOptionPane.showConfirmDialog(
              _vp, vsbb.getPanel(), "Set the space between each base", JOptionPane.OK_CANCEL_OPTION)
          != JOptionPane.OK_OPTION) {
  ...
  public void UISetNumPeriod() {
    if (_vp.getRNA().get_listeBases().size() != 0) {
      ...
      if (JOptionPane.showConfirmDialog(
              _vp, vnp.getPanel(), "Set new numbering period", JOptionPane.OK_CANCEL_OPTION)
          != JOptionPane.OK_OPTION) {
  ...
  public void UIEditBasePair() {
    if (_vp.isModifiable()) {
      ...
      if (mb.getElementStructure() != -1) {
        ...
        if (JOptionPane.showConfirmDialog(
                _vp, vbpt.getPanel(), "Set base pair L/W type", JOptionPane.OK_CANCEL_OPTION)
            != JOptionPane.OK_OPTION) {
          msbp.setEdge5(bck5);
          msbp.setEdge3(bck3);
  ...
  public void saveToPNG(String filename) throws ExceptionExportFailed {
    ...
    if (JOptionPane.showConfirmDialog(
            _vp, jpeg.getPanel(), "Set resolution", JOptionPane.OK_CANCEL_OPTION)
        == JOptionPane.OK_OPTION) {
  ...
  public void saveToJPEG(String filename) throws ExceptionJPEGEncoding, ExceptionExportFailed {
    ...
    if (JOptionPane.showConfirmDialog(
            _vp, jpeg.getPanel(), "Set resolution/quality", JOptionPane.OK_CANCEL_OPTION)
        == JOptionPane.OK_OPTION) {
      Double scale;
      if (jpeg.getScaleSlider().getValue() == 0) scale = 1. / 100.;
      else scale = jpeg.getScaleSlider().getValue() / 100.;
      BufferedImage myImage =
          new BufferedImage(
              (int) Math.round(_vp.getWidth() * scale),
              (int) Math.round(_vp.getHeight() * scale),
  ...
  public void UIPickSpecialBasesColor() {
    Color c =
        JColorChooser.showDialog(
            _vp, "Choose new special bases color", _vp.getNonStandardBasesColor());
    if (c != null) {
  ...
  public void UIPickGapsBasesColor() {
    Color c = JColorChooser.showDialog(_vp, "Choose new gaps bases color", _vp.getGapsBasesColor());
    if (c != null) {
  ...
  public void UIAbout() {
    ...
    JOptionPane.showMessageDialog(
        _vp,
        about,
        "About VARNA " + VARNAConfig.MAJOR_VERSION + "." + VARNAConfig.MINOR_VERSION,
  ...
  public void UIAnnotationEditFromAnnotation(TextAnnotation textAnnotation) {
    ...
    else vue = new VueAnnotation(_vp, textAnnotation, true, false);
    vue.show();
  }

  public void UIAnnotationAddFromStructure(
      TextAnnotation.AnchorType type, ArrayList<Integer> listeIndex) throws Exception {
    TextAnnotation textAnnot;
    ArrayList<ModeleBase> listeBase;
    VueAnnotation vue;
    switch (type) {
      case BASE:
        textAnnot = new TextAnnotation("", _vp.getRNA().get_listeBases().get(listeIndex.get(0)));
  ...
  public void UIAnnotationEditFromStructure(
      TextAnnotation.AnchorType type, ArrayList<Integer> listeIndex) {
    if (_vp.isModifiable()) {
      ModeleBase mb = _vp.getRNA().get_listeBases().get(listeIndex.get(0));
      TextAnnotation ta = _vp.getRNA().getAnnotation(type, mb);
      if (ta != null) UIAnnotationEditFromAnnotation(ta);
    }
  }

  public void UIAnnotationRemoveFromStructure(
      TextAnnotation.AnchorType type, ArrayList<Integer> listeIndex) {
    if (_vp.isModifiable()) {
      ModeleBase mb = _vp.getRNA().get_listeBases().get(listeIndex.get(0));
      TextAnnotation ta = _vp.getRNA().getAnnotation(type, mb);
      if (ta != null) UIAnnotationRemoveFromAnnotation(ta);
    }
  }
  ...
  public void UIAnnotationsAddPosition(int x, int y) {
    if (_vp.isModifiable()) {
      ...
      VueAnnotation annotationAdd = new VueAnnotation(_vp, (int) p.x, (int) p.y);
      annotationAdd.show();
    }
  }
  ...
  public void UIAddBP(int i, int j, ModeleBP ms) {
    if (_vp.isModifiable()) {
      ...
      tmp.add(ms);
      _vp.fireStructureChanged(
          new HashSet<ModeleBP>(_vp.getRNA().getAllBPs()), tmp, new HashSet<ModeleBP>());
    }
  }
  ...
  public void UIRemoveBP(ModeleBP ms) {
    if (_vp.isModifiable()) {
      ...
      tmp.add(ms);
      _vp.fireStructureChanged(
          new HashSet<ModeleBP>(_vp.getRNA().getAllBPs()), new HashSet<ModeleBP>(), tmp);
    }
  }
   ...
   *
   * @param index :the index of the selected base
   * @param newPos :the new xy coordinate, within the logical system of coordinates
   */
  public void UIMoveHelixAtom(int index, Point2D.Double newPos) {
    if (_vp.isModifiable() && (index >= 0) && (index < _vp.getRNA().get_listeBases().size())) {
      int indexTo = _vp.getRNA().get_listeBases().get(index).getElementStructure();
  ...
  public void UIRotateHelixAtom(int index, Point2D.Double newPos) {
    ...
    double base =
        (RNA.computeAngle(center, limitLoopRight) + RNA.computeAngle(center, limitLoopLeft)) / 2.0;
    ...
    while (pLimR < 0.0) pLimR += 2.0 * Math.PI;
    while (pHelR < pLimR) pHelR += 2.0 * Math.PI;
    while ((pNew < pHelR)) pNew += 2.0 * Math.PI;
    while ((pOld < pHelR)) pOld += 2.0 * Math.PI;
    while ((pHelL < pOld)) pHelL += 2.0 * Math.PI;
    while ((pLimL < pHelL)) pLimL += 2.0 * Math.PI;

    double minDelta = normalizeAngle((pLimR - pHelR) + 0.25);
    double maxDelta = normalizeAngle((pLimL - pHelL) - 0.25);
    while (maxDelta < minDelta) maxDelta += 2.0 * Math.PI;
    double delta = normalizeAngle(pNew - pOld);
    while (delta < minDelta) delta += 2.0 * Math.PI;
  ...
  public void UIRotateEverything(
      double delta, double base, double pLimL, double pLimR, Point h, Point ml) {
    Hashtable<Integer, Point2D.Double> backupPos = new Hashtable<Integer, Point2D.Double>();
