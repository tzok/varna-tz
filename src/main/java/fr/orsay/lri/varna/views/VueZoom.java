...
public class VueZoom implements ChangeListener {
  ...
  public VueZoom(VARNAPanel vp) {
    ...
    zoomSlider.setPreferredSize(new Dimension(250, zoomSlider.getPreferredSize().height));
    zoomSlider.addChangeListener(new ControleurZoom(this));

    JLabel zoomValueLabel = new JLabel(String.valueOf(_vp.getZoom()));
    zoomValueLabel.setPreferredSize(new Dimension(50, zoomValueLabel.getPreferredSize().height));
    zoomSlider.addChangeListener(new ControleurSliderLabel(zoomValueLabel, true));

    zoomAmountSlider =
        new JSlider(
            JSlider.HORIZONTAL,
            (int) (VARNAConfig.MIN_AMOUNT * 100),
            (int) (VARNAConfig.MAX_AMOUNT * 100),
            (int) (_vp.getZoomIncrement() * 100));
    // Turn on labels at major tick marks.
    ...
    zoomAmountSlider.setPreferredSize(
        new Dimension(200, zoomAmountSlider.getPreferredSize().height));

    JLabel zoomAmountValueLabel = new JLabel(String.valueOf(_vp.getZoomIncrement()));
    zoomAmountValueLabel.setPreferredSize(
        new Dimension(50, zoomAmountValueLabel.getPreferredSize().height));
    zoomAmountSlider.addChangeListener(new ControleurSliderLabel(zoomAmountValueLabel, true));
